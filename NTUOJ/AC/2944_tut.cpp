...
public:
    ...
    void printEdge(){
        for(edge &e:edg)PDE(e.u,e.v,e.cap,e.cst);
    }
};

minCostMaxFlow<int> solver(7000);
...
int main(){
    CPPinput;
    int n; cin>>n;
    // solver.addEdge(0,2,4,0);
    // solver.addEdge(n*2+1,1,4,0);
    solver.addEdge(6999,0,4,0);
    for(int i=1;i<=n;++i){
        cin>>a[i];
        solver.addEdge(i*2,i*2+1,1,0);
        solver.addEdge(i*2,i*2+1,8,1);
        // if(i<n)solver.addEdge(i*2+1,i*2+2,8,0);
        solver.addEdge(0,i*2,1,i-1);
        solver.addEdge(i*2+1,1,1,n-i);
    }
    for(int i=1;i<=n;++i){
        bool add=0,mns=0,mod=0;
        for(int j=i+1;j<=n;++j){
            if(!add && a[j]==a[i]+1)add=1,solver.addEdge(i*2+1,j*2,1,j-i-1);
            if(!mns && a[j]==a[i]-1)mns=1,solver.addEdge(i*2+1,j*2,1,j-i-1);
            if(!mod && a[j]%7==a[i]%7)mod=1,solver.addEdge(i*2+1,j*2,8,j-i-1);
        }
    }
    auto rt=solver.flow(6999,1);
    solver.printEdge();
    PDE(rt);
    cout<<n*4-rt.second<<endl;
}
