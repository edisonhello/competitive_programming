...
int main(){
    int ts,ks=0; rit(ts); while(ts--){
        int n,m,h,w; rit(n,m,h,w); 
        for(int i=1;i<=n;++i)for(int j=1;j<=m;++j)rit(a[i][j]);
        int L=1,R=n*m,mid;
        while(R>L){
            mid=(L+R)>>1;
            if([&]()->bool{
                for(int i=1;i<=n;++i)for(int j=1;j<=m;++j)p[i][j]=p[i-1][j]+p[i][j-1]-p[i-1][j-1]+(a[i][j]<=mid);
                for(int i=1;i+h-1<=n;++i)for(int j=1;j+w-1<=m;++j)if(p[i+h-1][j+w-1]-p[i-1][j+w-1]-p[i+h-1][j-1]+p[i-1][j-1]>=((w*h)>>1)+1)return 1;
                return 0;
            }())R=mid;
            else L=mid+1;
        }
