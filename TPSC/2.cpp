...
template<int I>ostream& operator<<(ostream &ostm,const mat<I> &m){ostm<<"\n["; for(int i=0;i<I;++i)ostm<<m[i]<<(i==I-1?"]\n":",\n "); return ostm;}
    ...
    mat<3> R,tk[3];
void init(){
    tk[0][0][0]=0.02; tk[0][0][1]=0.06; tk[0][0][2]=0.15;
    tk[0][1][0]=0.02; tk[0][1][1]=0.04; tk[0][1][2]=0.18;
    tk[0][2][0]=0.04; tk[0][2][1]=0.06; tk[0][2][2]=0.09;
    tk[1][0][0]=0.16; tk[1][0][1]=0.03; tk[1][0][2]=0.20;
    tk[1][1][0]=0.16; tk[1][1][1]=0.02; tk[1][1][2]=0.24;
    tk[1][2][0]=0.32; tk[1][2][1]=0.03; tk[1][2][2]=0.12;
    tk[2][0][0]=0.02; tk[2][0][1]=0.21; tk[2][0][2]=0.15;
    tk[2][1][0]=0.02; tk[2][1][1]=0.14; tk[2][1][2]=0.18;
    tk[2][2][0]=0.04; tk[2][2][1]=0.21; tk[2][2][2]=0.09;
}
vector<pair<ld,ll>> v;
int main(){
    init();
    int ts; cin>>ts; while(ts--){
        ...
        string str; cin>>str;    
        for(char &c:str)c-='0';
        if(str[0]==0)R[0][0]=0.03, R[0][1]=0.06, R[0][2]=0.12;
        if(str[0]==1)R[0][0]=0.24, R[0][1]=0.03, R[0][2]=0.28;
        if(str[0]==2)R[0][0]=0.03, R[0][1]=0.21, R[0][2]=0.12;
        for(int i=1;i<str.size();++i){
            R=R*tk[str[i]];
        }
        ld p=0;
        for(int i=0;i<3;++i)p+=R[0][i];
        //  cout<<fixed<<setprecision(100)<<p<<endl;
        v.emplace_back(p,num);
    }
    sort(v.begin(),v.end(),greater<pair<ld,ll>>());
    vector<int> ans;
    for(auto &i:v){
        if(abs(v[0].first-i.first)/abs(v[0].first)<1e-10)ans.push_back(i.second);
    }
