...
int main(){
    ll a,b,c,d,e,f; while(cin>>a>>b>>c>>d>>e>>f){
        if(a|b|c|d|e|f); else break;
        ll tot=f; f=0;
        tot+=e; a-=11*e; e=0; a=max(a,0ll);
        tot+=d; if(b>5*d)b-=5*d,d=0; else b-=5*d,d=0,a-=-b*4,a=max(a,0ll),b=0;
        tot+=c/4; c%=4; if(c){
            ++tot;
            if(c==1){
                if(b>5)b-=5;
        ...
        tot+=b/9; b%=9; if(b)++tot,b=9-b,a-=4*b,b=0,a=max(a,0ll);
        tot+=a/36; a%=36; if(a)++tot; a=0;
        cout<<tot<<endl;
    }
}
