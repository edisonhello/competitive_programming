...
#define pqpq PQ<int,vector<int>,less<int>> 
        int n; cin>>n; pqpq *pq=new pqpq(),*tmp=new pqpq(); int ans=0,duo=0;
        for(int i=0,t;i<n;++i)cin>>t,pq->push(t);
        while(pq->size()>1u){
            while(pq->size()>1u){
                int tt=pq->top(); pq->pop(); tt+=pq->top(); pq->pop();
                tmp->push(tt);
            }
            if(pq->size())duo+=pq->top(),pq->pop();
            swap(pq,tmp); ans+=duo;
        }
        cout<<ans<<endl;
        while(pq->size())pq->pop();
        while(tmp->size())tmp->pop();
        delete pq; delete tmp;
    }
}
