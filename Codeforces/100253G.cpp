...
string simplify(string s) {
    ...
    int idx = 0;
    int n = s.size();
    while (idx < n && s[idx] == '(') t += s[idx], ++idx;
    while (idx < n) {
        bool first = true;
        while (idx < n && (isalpha(s[idx]) || isdigit(s[idx]) || (first && s[idx] == '-'))) ++idx, first = false;
        t += 'a';
        while (idx < n && (s[idx] == ')')) t += s[idx], ++idx;
        if (idx < n && (s[idx] == '+' || s[idx] == '-' || s[idx] == '*')) t += s[idx], ++idx;
        while (idx < n && (s[idx] == '(')) t += s[idx], ++idx;
        // cout <<t << endl;
    }
    return t;
...
node *dfs(int l, int r, string &s) {
    if (l > r) return 0;
    if (op[l] && op[l] == r) return dfs(l + 1, r - 1, s);
    if (l == r) return &pool[++poolitr];
    // cout << "Dfs: " << l << " " << r << endl;
    stack<char> ops;
    stack<node*> opr;
    ...
    while (l <= r) {
        if (s[l] == '+' || s[l] == '-') {
            ...
            // cout << "Jump from " << l << " to " << op[l] +1 << endl;
            l = op[l] + 1;
        } else if (s[l] == 'a') {
            opr.push(&pool[++poolitr]);
            ++l;
        } else if (s[l] == '*') {
            if (ops.size() && ops.top() == '*') {
                ops.pop();
