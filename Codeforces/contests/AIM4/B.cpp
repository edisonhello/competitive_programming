...
int main(){
    ...
    ans+=n*m;
    for(int i=0;i<n;++i){
        ans+=(1ll<<rcnt[i])-1-rcnt[i];
        ans+=(1ll<<(m-rcnt[i]))-1-(m-rcnt[i]);
        PDE1(ans);
    }
    PDE1(ans);
    for(int i=0;i<m;++i){
        ans+=(1ll<<ccnt[i])-1-ccnt[i];
        ans+=(1ll<<(n-ccnt[i]))-1-(n-ccnt[i]);
    }
    cout<<ans<<endl;

}
// 29739528 13:38 AC->skipped
// 29745488 43:17 AC
