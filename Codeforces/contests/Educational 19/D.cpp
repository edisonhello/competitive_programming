...
int n,l[100005],r[100005],v[100005],pa[100005],ans;
set<int> f;
...
int dfs(int now,int L,int R){
    ...
    dfs(l[now],L,min(R,v[now]));
    dfs(r[now],max(L,v[now]),R);
}

int main(){
    int n;cin>>n;
    for(int i=1;i<=n;++i)cin>>v[i]>>l[i]>>r[i],pa[l[i]]=i,pa[r[i]]=i;
    int root=-1; for(int i=1;i<=n;++i)if(!pa[i])root=i; assert(root!=-1);
    dfs(root,0,1000000000);
    for(int i=1;i<=n;++i)if(f.find(v[i])==f.end())++ans;
    cout<<ans<<endl;
}
// 26385368 34:22 AC <- Hacked
// 26391006 88:35 AC
