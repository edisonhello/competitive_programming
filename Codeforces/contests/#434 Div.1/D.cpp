...
int main(){
    ...
    while(m>=2){
        if(d1.size()){
            ...
            assert(G[u].size()==1u);
            int id=*G[u].begin(); G[u].clear();
            pii e=Gs[id];
            auto it=avile.find(id);
            if(it==avile.end())continue;
            avail.erase(it);
            int v=e.first==u?e.second:e.first;
            if(G[v].size()==1u)continue;
            int iid=*G[v].begin(); G[v].erase(G[v].begin());
            pii ee=Gs[iid];
