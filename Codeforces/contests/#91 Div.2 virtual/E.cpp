...
init();
    ...
    PDE(*upper_bound(luc.begin(),luc.end(),fix));
    vector<ll> candi;
    for(int i=fix+1,ptr=0;i<=n;++i,++ptr)ori[ptr]=i,candi.push_back(i);
    PDE(candi);
    --k;
    ...
    for(int i=0;i<move;++i)if(*lower_bound(luc.begin(),luc.end(),a[i])==a[i] && *lower_bound(luc.begin(),luc.end(),fix+i+1)==fix+i+1)++ans;
    cout<<ans<<endl;
}
