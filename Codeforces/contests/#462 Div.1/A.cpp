...
int a[2345],revmax[2345][2345],posmax[2345][2345];
...
int main(){
    ...
    for(int i=1,j;i<=n;i=j){
        ...
        v.emplace_back(j-i,a[i]);
    }
    if(v.size()<=3u)return cout<<n<<endl,0;
    if(v.size()==4u && v[0].second==1)return cout<<n<<endl,0;
    int vz=v.size();
    for(int j=vz-1;j>=0;--j){
        revmax[j][j]=v[j].first;
        for(int i=j-1;i>=0;--i){
            revmax[i][j]=revmax[i+1][j];
            if(v[i].second==2)revmax[i][j]=max(revmax[i][j],v[i].first+v[i+1].first);
            revmax[i][j]=max(revmax[i][j],v[i].first);
        }
    }
    for(int i=0;i<vz;++i){
        posmax[i][i]=v[i].first;
        for(int j=i+1;j<vz;++j){
            posmax[i][j]=posmax[i][j-1];
            if(v[j].second==2)posmax[i][j]=max(posmax[i][j],v[i].first+v[i+1].first);
            posmax[i][j]=max(posmax[i][j],v[i].first);
        }
    }
    int ans=0;
    for(int i=0;i<vz;++i){
        for(int j=0;j<vz;++j){
            ans=max(ans,revmax[i][j]+_get(0,i-1)+_get(j+1,vz-1));
            if(i){
                ans=max(
            }
        }
