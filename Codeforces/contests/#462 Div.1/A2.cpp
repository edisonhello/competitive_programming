...
int solve(int n){
    ...
    PDE(v);
    if(v.size()<=3u)return n;
    if(v.size()==4u && v[0].second==1)return n;
    int vz=v.size();
    int ans=0;
    for(int i=0;i<vz-1;++i){
        if(v[i].second==2){
            ans=max(ans,v[i].first+v[i+1].first+(i?v[i-1].first:0)+(i+2<vz?v[i+2].first:0));
        }
        else{
            for(int j=i-1;j>=0;--j){
                if(v[j].second==1){
                    ans=max(ans,v[i].first+v[i+1].first+v[j].first);
                }
                else{
                    ans=max(ans,v[i+1].first+v[j].first+v[j+1].first);
                }
            }
            for(int j=i+2;j<vz;++j){
                if(v[j].second==1){
                    ans=max(ans,v[i].first+v[j].first+v[j-1].first);
                }
                else{
                    ans=max(ans,v[i].first+v[i+1].first+v[j].first);
                }
    ...
    for(int i=1;i<=n;++i){
        for(int j=i;j<=n;++j){
            ...
            for(int i=0;i<v.size();++i){
                nans=max(nans,v[i].first);
                if(v[i].second==1 && i+1<v.size())nans=max(nans,v[i].first+v[i+1].first);
            }
            if(nans>ans){
                ans=nans;
                gfx=i,gfy=j;
