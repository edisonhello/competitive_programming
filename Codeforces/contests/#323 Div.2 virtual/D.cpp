...
int samp[105],a[100505],cnt[505];
int main(){
    CPPinput;
    int n,t; cin>>n>>t;
    for(int i=1;i<=n;++i)cin>>samp[i];
    if(t<=600){
        for(int i=1;i<=n*t;++i)a[i]=samp[(i-1)%n+1];
        n=n*t;
        vector<int> lis;
        for(int i=1;i<=n;++i){
        ...
        for(int i=1;i<=n*505;++i)a[i]=samp[(i-1)%n+1];
        vector<int> lis;
        for(int i=1;i<=n*505;++i){
        ...
        int ans=lis.size();
        PDE(lis,ans);
        for(int i=1;i<=n;++i)++cnt[samp[i]];
        for(int i=1;i<=300;++i)ans=max(ans,(int)lis.size()+cnt[i]*(t-505));
        cout<<ans<<endl;
    }
}
