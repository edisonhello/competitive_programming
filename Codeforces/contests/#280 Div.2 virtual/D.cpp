...
ll hits(ld t){
    ld xt=(ld)1/(ld)x;
    ld yt=(ld)1/(ld)y;
    // PDE4(t,xt,t/xt,(int)(t/xt));
    ll rt=(ll)(t/xt)+(ll)(t/yt);
    // PDE2(t,rt);
    return rt;
}
...
int main(){
    ...
    while(n--){
        ...
        // cout<<"L: "<<fixed<<setprecision(12)<<L<<endl;
        // PDE1(hits(L));
        // PDE4(L,xt,L/xt,floor(L/xt+eps1));
        // PDE4(L,yt,L/yt,floor(L/yt+eps1));
        ld aftx=L-xt*floor(L/xt+eps1);
        ld afty=L-yt*floor(L/yt+eps1);
        // PDE2(aftx,afty);
        if(fabs(L-1)<eps)cout<<"Both\n";
        else if(aftx<afty)cout<<"Vanya\n";
        else cout<<"Vova\n";
    }
}
// 25203189 30:07 WA 6
