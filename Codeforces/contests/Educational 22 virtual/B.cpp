...
int main(){
    ...
    set<ll> s; s.insert(l-1); s.insert(r+1);
    int i,j;
    ll cx,cy,nx,ny;
    for(cx=1;;){
        nx=cx;
        for(cy=1;;){
            ...
            if(nx+ny<0)break;
            if(nx+ny>r)break;
            if(l<=nx+ny && nx+ny<=r)s.insert(nx+ny);
            cy=ny*y; if(cy/y!=ny)break;
        }
        cx=nx*x; if(cx/x!=nx)break;
    }
    ll lon=0;
    PDE1(s);
    auto it=s.begin(), it2=s.begin(); ++it2;
    for(;it2!=s.end();++it,++it2){
        lon=max(lon,*it2-*it-1);
    }
    cout<<lon<<endl;
}
// 28905605 30:16 CE (test for __int128)
// 28905857 42:44 AC
