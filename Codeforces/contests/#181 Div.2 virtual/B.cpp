...
int main(){
    ...
    for(int i=1;i<=n;++i)if(djs.size(i)>3)return cout<<"-1"<<endl,0;
    bitset<3499> out;
    for(int i=1;i<=n;++i){
        if(djs.size(i)==3 && !out[i]){
            for(int j=1;j<=n;++j){
                if(djs.C(i,j)){
                    thr.push_back(j);
                    out[j]=1;
        ...
        if(djs.size(i)==1)one.push_back(i);
    }
    PDE(thr,two,one);
    if(two.size()>one.size())return cout<<"-1"<<endl,0;
    for(int i=0;i<thr.size();++i){
        cout<<thr[i]<<" ";
        if(i%3==2)cout<<endl;
    }
    while(two.size()){
        cout<<two.back().X<<" "<<two.back().Y<<" "<<one.back()<<endl;
