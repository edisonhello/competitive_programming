...
int meow(string &s){
    if(s.size()<=1u)return 0;
    map<char,int> mp;
    for(char c:s)++mp[c];
    if(mp.size()<=1u)return 0;
    auto it=mp.begin();
    int num=it->second;
    ++it;
    return min(num,it->second);
}

int main(){
    string s; cin>>s;
    int k,ptr=12; cin>>k; while(k--){
        string forb; cin>>forb;
        grp[forb[0]]=ptr;
        grp[forb[1]]=ptr;
        ++ptr;
    }
