...
main(){
    CPPinput;
    int a,c; cin>>a>>c;
    vector<int> av=de(a),cv=de(c);
    PDE(av,cv);
    vector<int> bv(max(av.size(),cv.size()));
    for(int i=0;i<max(av.size(),cv.size());++i){
        int cc=i>=cv.size()?3:cv[i]+3;
        int aa=i>=av.size()?0:av[i];
        int bb=cc-aa;
        bb%=3;
        bv[i]=bb;
    }
    int b=0;
    for(int i=0,base=1;i<bv.size();++i,base*=3){
        b+=bv[i]*base;
    }
    cout<<b<<endl;
}
