...
bool almost(ld a,ld b){
    return abs(a-b)<eps;
}

bool issqu(vector<pii> &v){
    vector<ld> len;
    for(int i=0;i<4;++i){
        for(int j=i+1;j<4;++j){
            len.push_back(dis(v[i],v[j]));
        }
    }
    sort(len.begin(),len.end());
    bool ok=1;
    for(int i=0;i<3;++i)if(!almost(len[i],len[i+1]))ok=0;
    for(int i=4;i<5;++i)if(!almost(len[i],len[i+1]))ok=0;
    if(!almost(len[3]*sqrt(2),len[4]))ok=0;
    PDE("squ",len,ok);
    return ok;
}
...
bool isrec(vector<pii> &v){
    ...
    sort(len.begin(),len.end());
    bool ok=1;
    for(int i=0;i<1;++i)if(!almost(len[i],len[i+1]))ok=0;
...
int main(){
    ...
    for(int i=0;i<(1<<8);++i){
        if(bitset<8>(i).count()==4){
            vector<pii> squ,rec;
            for(int j=0;j<8;++j){
                if(i&(1<<j))squ.push_back(pts[j]);
                else rec.push_back(pts[j]);
            }
            if(issqu(squ) && isrec(rec)){
                cout<<"YES"<<endl;
                for(int j=0;j<8;++j){
                    if(i&(1<<j))cout<<j+1<<" ";
                } cout<<endl;
