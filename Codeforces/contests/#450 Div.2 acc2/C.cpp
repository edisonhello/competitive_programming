...
int main(){
    ...
    st.clear();
    int mx=-71717171,ii=1;
    for(int i=1;i<=n;++i){
        auto it=st.lower_bound(a[i]);
        int delta=-(int)(it==st.end() || (i==1&&a[i+1]>a[i]))+attr[a[i]];
        PDE(i,attr[a[i]],delta);
        if(delta>mx || (delta==mx && a[i]<a[ii]))mx=delta,ii=i;
        st.insert(a[i]);
    }
    cout<<a[ii]<<endl;
}
