...
ll cnt(ll ch,ll ot){
    ll invot=inv(ot);
    return ch*invot%mod*invot%mod;
}

int main(){
    int k; ll pa_,pb_; cin>>k>>pa_>>pb_;
    ll pa=pa_*inv(pa_+pb_)%mod,pb=pb_*inv(pa_+pb_)%mod;
    ll a_explen=cnt(pa,pb),b_explen=cnt(pb,pa);
    ll explen=b_explen;
    for(int i=1;i<=k;++i)explen+=a_explen+b_explen;
    cout<<explen%mod<<endl;
}
