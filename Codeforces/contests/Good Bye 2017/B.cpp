...
bool ok(int sx,int sy,int ex,int ey){
    PDE(match[0],match[1],match[2],match[3]);
    for(char c:com){
        int dir=match[c];
        if(dir==0){
            if(sx==0)return 0;
            if(mp[sx-1][sy]=='#')return 0;
            --sx;
        }
        else if(dir==1){
            if(sy==0)return 0;
            if(mp[sx][sy-1]=='#')return 0;
            --sy;
        }
        else if(dir==2){
            if(sx==n-1)return 0;
...
int main(){
    ...
    cin>>com;
    for(char &c:com)c-='0';
    for(int i=0;i<4;++i)match[i]=i;
    int ans=0;
    int sx,sy,ex,ey;
    init(sx,sy,ex,ey);
    do{
        if(ok(sx,sy,ex,ey))++ans;
    }while(next_permutation(match,match+4));
    cout<<ans<<endl;
}
// 33770330 12:36 AC
