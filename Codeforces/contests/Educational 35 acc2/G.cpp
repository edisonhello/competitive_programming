...
void modify(node *now,int l,int r,int ml,int mr,int x,int y,int t){
    if(ml<=l && r<=mr){
        auto it=now->ope.find(x);
        if(it==now->ope.end())now->ope[x]=pii(y,t);
        else
    }
    if(r<ml || mr<l)return;
}
...
int main(){
    ...
    int m; rit(m); for(int i=1;i<=m;++i){
        int l,r,x,y; rit(l,r,x,y);
        modify(root,1,n,l,r,x,y,i);
    }
    travel(root,1,n);
}
