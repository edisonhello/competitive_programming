...
int main(){
    ...
    for(int i=1;i<=k;++i){
        ...
        app[t]=1;
        PDE(st);
        if(t!=nowmin && (st.size() && t>st.top()))JIZZ();
        if(t==nowmin){
            while(app[nowmin])++nowmin;
            while(st.size() && st.top()<=nowmin)st.pop();
        }
        else st.push(t);
        ans.push_back(t);
    }
    ...
    while(ans.size()<n){
        int start=nowmin;
        while(!app[start])++start; --start;
        PDE(start,nowmin);
        for(int i=start;i>=nowmin;--i){
            app[i]=1;
            ans.push_back(i);
        }
        nowmin=start;
