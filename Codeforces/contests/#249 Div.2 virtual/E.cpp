...
int main(){
    ...
    for(int i=0;i<n;++i){
        for(int j=0;j<m;++j){
            if(s[i][j]!='0')continue;
            set<int> st;
            if(i)st.insert(s[i-1][j]);
            if(j)st.insert(s[i][j-1]);
            if(i<n-1)st.insert(s[i+1][j]);
            if(j<m-1)st.insert(s[i][j+1]);
            if(i&&j)st.insert(s[i-1][j-1]);
            if(i&&j<m-1)st.insert(s[i-1][j+1]);
            if(i<n-1&&j)st.insert(s[i+1][j-1]);
            if(i<n-1&&j<m-1)st.insert(s[i+1][j+1]);
            if(st.find('1')==st.end())s[i][j]='1';
            else if(st.find('2')==st.end())s[i][j]='2';
            else if(st.find('3')==st.end())s[i][j]='3';
            else if(st.find('4')==st.end())s[i][j]='4';
            else JIZZ();
        }
    }
