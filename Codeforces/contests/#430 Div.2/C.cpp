...
void dfs(int now,int fa){
    if(now>1){
        dp[1][now]=max(gcd(dp[0][fa],0),gcd(dp[1][fa],a[now]));
        dp[0][now]=gcd(dp[0][fa],a[now]);
    }
    else dp[0][now]=a[now],dp[1][now]=0;
    for(int i:G[now]){
        if(i==fa)continue;
        dfs(i,now);
    }
...
int main(){
    ...
    dfs(1,1);
    for(int i=1;i<=n;++i)printf("%d ",max(dp[0][i],dp[1][i]));
}
// 29878635 25:18 WA 4
// 29882738 39:21 WA 4
