...
int segcnt[100005],cnt[100005],lislen[100005],ldslen[100005],bit[100005];
vector<int> event[100005];

void add(int x,int v){
    for(;x<100005;x+=lowbit(x))bit[x]=max(bit[x],v);
}
...
int main(){
    ...
    for(int i=1;i<=m;++i){
        lislen[i]=query(a[i])+1;
        add(a[i],lislen[i]);
    }
    memset(bit,0,sizeof(bit));
    for(int i=m;i>=1;--i){
        ldslen[i]=query(a[i])+1;
        add(a[i],ldslen[i]);
    }
    int mx=0;
    for(int i=1;i<=m;++i){
        mx=max(mx,lislen[i]+ldslen[i]-1);
        PDE(i,lislen[i],ldslen[i]);
    }
    cout<<mx<<endl;
}
