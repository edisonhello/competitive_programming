...
int test(vector<string> &v){
    ...
    for(int i=1;i<n;++i){
        ...
        int uniq=0;
        for(int i=0;i<26;++i)if(cnt[i]==1)++uniq;
        mx=max(mx,uniq);
    }
    return mx;
}
...
int main(){
    ...
    for(int i=0;i<s.size();++i){
        string ns=s.substr(s.size()-i,i)+s.substr(0,s.size()-i);
        v[ns[0]-'a'].push_back(ns);
    }
    for(int i=0;i<26;++i)PDE(i,v[i]);
    ld ans=0;
    for(int i=0;i<26;++i){
        ans+=test(v[i]);
    }
    cout<<fixed<<setprecision(12)<<ans/s.size()<<endl;
}
