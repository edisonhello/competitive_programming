...
int get(set<int> &st,int x){
    return x-*(--st.upper_bound(x));
}
int main(){
    ios_base::sync_with_stdio(0); cin.tie(0);
    set<int> up,lft;
    int n,q; cin>>n>>q; up.insert(0), lft.insert(0);
    while(q--){
        int x,y; string dir; cin>>x>>y>>dir;
        if(dir[0]=='U'){
            cout<<(up.find(x)==up.end()?get(lft,y):0)<<endl;
            up.insert(x);
        }
        else{
            cout<<(lft.find(y)==lft.end()?get(up,x):0)<<endl;
            lft.insert(y);
        }
    }
}
// 33091172 115:27 WA 5
