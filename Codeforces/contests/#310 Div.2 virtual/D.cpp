...
pair<pll,int> seg[200005];
int ans[200005];
int main(){
    int n,m; rit(n,m);
    for(int i=1;i<=n;++i)rit(l[i],r[i]);
    for(int i=0;i<m;++i)rit(b[i].X),b[i].Y=i+1;
    for(int i=2;i<=n;++i){
        seg[i-2]=make_pair(make_pair(l[i]-r[i-1],r[i]-l[i-1]),i-2);
    }
    sort(seg,seg+n-1,[](const pair<pll,int> &a,const pair<pll,int> &b){return a.X.X==b.X.X?a.X.Y<b.X.Y:a.X.X<b.X.X;});
    sort(b,b+m);
    set<pair<ll,int>> st;
    int ptr=0; for(int i=0;i<m;++i){
        while(ptr<n-1 && seg[ptr].X.X<=b[i].X)st.insert({seg[ptr].X.Y,seg[ptr].Y}),++ptr;
        if(st.size()){
            // cout<<i<<" "<<st<<endl;
            auto it=st.upper_bound({b[i].X,-9});
            if(it!=st.end())ans[it->Y]=b[i].Y,st.erase(it);
        }
    }
    if(st.size())cout<<"No"<<endl,exit(0);
    cout<<"Yes"<<endl; for(int i=0;i<n-1;++i)cout<<ans[i]<<" "; cout<<endl;
}
// 33090344 46:55 WA 10
// 33090597 66:46 WA 10
// 33090635 70:28 WA 10
// 33090994 99:29 WA 10
// 33091014 101:23 WA 47
