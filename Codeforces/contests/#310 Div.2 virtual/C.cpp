...
pair<int,int> at[100005];
int main(){
    int n,k; cin>>n>>k;
    for(int i=1,t;i<=k;++i){
        cin>>t; chain[i].resize(t);
        for(int j=0;j<t;++j)cin>>chain[i][j],at[chain[i][j]]=pii(i,j);
    }
    int freee=0;
    pii pos(at[1].X,at[1].Y+1);
    while(pos.Y<chain[pos.X].size() && chain[pos.X][pos.Y]==chain[pos.X][pos.Y-1]+1)++freee,++pos.Y;
    cout<<n-k-freee+(n-freee-1)<<endl;
    /* int seg=0;
    for(int i=1;i<=k;++i){
        ++seg;
        for(int j=chain[i].size()-2,ok=1;j>=0;--j){
            ++seg;
            if(chain[i][j]==chain[i][j+1]-1 && ok)--seg;
            else ok=0;
        }
    }
    
    cout<<(seg-1)*2-k+1<<endl; exit(0);
    *//* int ans=0; pii pos(at[n].X,at[n].Y+1);
    for(int i=n;i>=1;--i){
        --pos.Y;
        if(pos.Y>=0 && chain[pos.X][pos.Y]==i)continue;
        else{
