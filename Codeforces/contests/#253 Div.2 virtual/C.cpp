...
int main(){
    ...
    PDE1(allin);
    while(allin>1){
        int cxt=0,cyt=0,mxx=0,mxy=0,mxxi=-1,mxyi=-1;
        for(int i=0;i<5;++i)if(cx[i]){
            ++cxt;
            if(mxx<cx[i]){
                mxx=cx[i];
                mxxi=i;
        ...
        PDE4(mxx,mxy,cyt,cxt);
        if((mxx>mxy && cxt!=1)||cyt==1){
            // cout<<"meow"<<endl;
            for(int i=0;i<5;++i){
                if(cnt[mxxi][i]){
                    cnt[mxxi][i]=0;
