...
vector<int> ans[200006];
deque<vector<int>*> end0,end1,unused;

int main(){
    CPPinput;
    string s; cin>>s;
    for(int i=0;i<200006;++i)unused.push_back(&ans[i]);
    for(int i=0;i<s.size();++i){
        if(s[i]=='0'){
            vector<int> *it;
            if(end1.size()){it=end1.front(); end1.pop_front();}
            else{it=unused.front(); unused.pop_front();}
            it->push_back(i+1);
            end0.push_back(it);
        }
        else{
