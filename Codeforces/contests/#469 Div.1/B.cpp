...
inline ll get(ll n,ll x){
    ll offset=0,width=2,front=0,step=0,numbers=(n+width/2)/width;
    while(1){
        // PDE(offset,width,front,step,numbers);
        if((x-offset)%width==0){
            return front+(x-offset)/width+1;
        }
        else{
            if((1ll<<step)&n){
                offset+=width*3/2;
