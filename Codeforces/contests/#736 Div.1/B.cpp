...
int64_t query(int o, int l, int r, int ql, int qr) {
  ...
  return __gcd(query(lc, l, mid, ql, qr), query(rc, mid + 1, r, ql, qr));
}
...
void solve() {
  ...
  for (int i = 1; i < n; ++i) d[i] = abs(v[i] - v[i + 1]);

  build(0, 1, n - 1);

  auto get = [&] (int l, int r) { return query(0, 1, n - 1, l, r); };


  int ans = 1;
  int L = 1, R = 1;

  for (int i = 1; i < n; ++i) if (d[i] > 1) ans = 2;
  ...
  while (true) {
    ...
    if (get(L, R) > 1) ans = max(ans, R - L + 2);
    PDE(L, R, get(L, R), ans);
  }
