...
int main(){
    unsigned ll n,m;cin>>n>>m;
    if(m>=n){
        cout<<n<<endl;
        return 0;
    }
    ll lft=n-m;
    ll lday=(ll)sqrt((ld)lft);
    for(;;++lday){
        PDE1(lday*(lday+1)/2);FLH;
        if(lday*(lday+1)/2>=lft)break;
    }
    cout<<m+lday<<endl;
}
//
...
// day 3 (n-m-6)
// 25508928 20:52 AC
//          61:42 successful hack
//          72:42 successful hack
//          89:42 successful hack
//          91:42 unsuccessful hack
//          97:50 hacked
