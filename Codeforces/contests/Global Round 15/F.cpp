...
int main() {
  ...
  auto seg_ok = [&](int l, int r) -> int64_t {
    int cnt = 0;
    for (int x = r + 1; x; x -= x & -x) cnt += bit_ok[x];
    for (int x = l - 1 + 1; x; x -= x & -x) cnt -= bit_ok[x];
    if (cnt != r - l + 1) return -1;
    int64_t val = 0;
    for (int x = r + 1; x; x -= x & -x) val += bit_sum[x];
    for (int x = l - 1 + 1; x; x -= x & -x) val -= bit_sum[x];
  ...
  while (true) {
    ...
    int fly_L = nxi, fly_R = n;
    while (fly_L < fly_R) {
      int M = (fly_L + fly_R + 1) >> 1;
      int val = seg_ok(nxi, M);
      if (val != -1)
        fly_L = M;
      else
        fly_R = M - 1;
    ...
    if (fly_L != nxi) {
      PDE(at);
      int val = seg_ok(nxi, fly_L);
      WalkUntil(ps[fly_L].x);
      (tot += val) %= mod;
      at = ps[fly_L].x;
      PDE(nxi, fly_L, at);
    } else {
      ...
      if (state == 1) {
        ...
        if (prev_come[nxi] >= 0) {
          ...
          for (int x = nxi + 1; x < 200005; x += x & -x) bit_ok[x]++;
          for (int x = nxi + 1; x < 200005; x += x & -x)
            (bit_sum[x] += needs_back[nxi]) %= mod;
        }
      }
    }
