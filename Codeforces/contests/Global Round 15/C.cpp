...
int main() {
  ...
  while (t--) {
    ...
    for (int i = 0; i < n; ++i) {
      for (int j = i + 1; j < n; ++j) {
        pair<int, int> a = minmax(lns[i].first, lns[i].second);
        pair<int, int> b = minmax(lns[j].first, lns[j].second);
        if (b.first < a.first) swap(a, b);
        if (a.second > b.first && a.second < b.second) ++crx;
      }
    }
