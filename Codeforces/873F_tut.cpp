...
int R(int idx,int x){cout<<idx<<" "<<x<<endl; return lcpa[idx]<x?idx:R(right[idx],x);}
...
int main(){
    ...
    long long ans=0;
    cout<<"do LR"<<endl;
    left[0]=-1; for(int i=1;i<n;++i)left[i]=L(i-1,lcpa[i]),cout<<i<<endl;
    cout<<"L"<<endl;

    right[n-1]=n; for(int i=n-2;i>=0;--i)right[i]=R(i+1,lcpa[i]),cout<<i<<endl;
    for(int i=0;i<n;++i)forpre[i]=(i?forpre[i-1]:0)+(forbid[i]=='1');
    for(int i=0;i<n;++i)ans=max(ans,1ll*(right[i]-left[i]+1)*lcpa[i]);
    cout<<ans<<endl;
