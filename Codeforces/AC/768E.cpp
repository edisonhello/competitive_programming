...
int SG[100];
map<pair<int,ll>,int> msg;

int d(int x,ll abl){
    auto it=msg.find({x,abl});
    if(it!=msg.end())return it->Y;
    set<int> st;
    for(int i=1;i<=x;++i){
        if(abl & (1ll<<i)){
            abl^=(1ll<<i);
...
int main(){
    init();
    // for(int i=1;i<=60;++i)cout<<SG[i]<<" ";cout<<endl;FLH;
    int n;cin>>n;int a=0;while(n--){
        int t;cin>>t;a^=SG[t];
    }
    if(a)cout<<"NO"<<endl;
    else cout<<"YES"<<endl;
