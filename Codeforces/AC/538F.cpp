...
int a[200006],b[400006],o[200006];
void add(int x){
    while(x<=400005){
        b[x]++;
        x+=lowbit(x);
...
int main(){
    ...
    vector<qry> q;
    for(int i=1;i<=n;++i){
        int pt=1,l=2,r=l+i-1;
        while(l<=n){
            q.push_back({i,r,a[pt]-1});
            q.push_back({-i,l-1,a[pt]-1});
            ++pt; l+=i,r+=i;
        }
    }
    sort(q.begin(),q.end(),[](const qry &a,const qry &b){return a.pos<b.pos;});
    int pt=1;
    for(auto &i:q){
        PDE2(pt,i.pos);
        while(pt<=i.pos && pt<=n)add(a[pt++]);
        o[abs(i.i)]+=qer(i.x)*(i.i>0?1:-1);
    }
    for(int i=1;i<n;++i)cout<<o[i]<<" "; cout<<endl;
}
