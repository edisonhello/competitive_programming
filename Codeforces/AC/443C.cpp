...
int main(){
    ...
    for(int i=0;i<n;++i){
        string s; cin>>s;
        info.insert(make_pair(ps(s[0]),s[1]-'0'));
    }
    int ans=10000;
    for(int i=0,cnt=0;i<1024;++i,cnt=0){
        memset(hint,0,sizeof(hint));
        for(int j=0;j<10;++j){
            if(i&(1<<j)){
                ++cnt;
                if(j<5){
                    for(int i=1;i<=5;++i)hint[i][j+1]+=1<<j;
                }
                else{
                    for(int i=1;i<=5;++i)hint[j-4][i]+=1<<j;
                }
            }
        }
        set<int> st;
        bool ok=1;
        for(auto i:info){
            if(st.find(hint[i.X][i.Y])!=st.end())ok=0;
            st.insert(hint[i.X][i.Y]);
        }
        if(ok){
            ans=min(ans,cnt);
        }
