...
int main(){
    ...
    for(int i=0;i<s.length();++i){
        for(int j=0;i+j<s.length();++j){
            int l=j,r=j+i;
            if(l==r)dp[l][r]=1;
            else if(r==l+1 && s[l]==s[r])dp[l][r]=2;
            else if(r>l+1){
                if(s[l]!=s[r] || !dp[l+1][r-1]);
                else{
                    dp[l][r]=min(dp[l][int(ceil(((ld)(l+r))/2))-1],dp[int(floor(((ld)(l+r))/2))+1][r])+1;
                }
            }
        }
    }
    if(DEBUG)for(int i=0;i<s.length();++i){
