...
int a[200005],ans[200005],in[200005];
vint G[200005];
int gcd(int a,int b){
    if(!a)return b;
    if(!b)return a;
...
vector<int> de(int x){
    ...
    for(int j=1;j*j<=x;++j){
        if(x%j)continue;
        rt.push_back(j);
        if(j*j!=x)rt.push_back(x/j);
    } return rt;
}
void dfs(int now,int fa,int x,int ned){
    for(int i:G[now]){
        if(i==fa)continue;
        vector<int> D=de(a[i]);
        ans[i]=x;
        for(int &j:D)if(++in[j]>=ned)ans[i]=max(ans[i],j);
        dfs(i,now,gcd(x,a[i]),ned+1);
        for(int &j:D)--in[j];
    }
}
int main(){
