...
ll n,x1,y1,x2,y2,r1,r2,mr=1e17;
ll x[2222],y[2222];
...
int main(){
    ...
    for(int i=0;i<n;++i){
        ...
        PDE2(i,r1); r2=0;
        for(int j=0;j<n;++j){
            if((x[j]-x1)*(x[j]-x1)+(y[j]-y1)*(y[j]-y1)<=r1)continue;
    ...
    for(int i=0;i<n;++i){
        ...
        r1=0;
        for(int j=0;j<n;++j){
            if((x[j]-x2)*(x[j]-x2)+(y[j]-y2)*(y[j]-y2)<=r2)continue;
            r1=max(r1,(x[j]-x1)*(x[j]-x1)+(y[j]-y1)*(y[j]-y1));
            PDE2(j,r1);
        }
        mr=min(mr,r1+r2);
    }
