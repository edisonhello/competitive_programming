...
#ifdef COMPILE_ERROR
p/s=k => ([i:j])
(p[j]/p[i-1])/(s[j]-s[i-1])=k
p[j]=(s[j]-s[i-1])*k*p[i-1];
p[j]=s[j]*k*p[i-1]-s[i-1]*k*p[i-1];
#endif
...
int main(){
    ...
    for(int i=1;i<=n;++i)if(a[i]>1)good.pb(i);
    good.pb(n+1);
    if(0){
        ll ans=0;
        for(int i=1;i<=n;++i){
