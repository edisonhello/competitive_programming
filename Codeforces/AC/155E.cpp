...
unsigned long long val[1000006];
unsigned long long hh[1000006];
int deg[1000006];
map<unsigned long long,long long> mp;
int main(){
    int n,m; rit(n,m);
    for(int i=1;i<=n;++i)val[i]=((val[i-1]*884453ll)^8846128819945775ll)*8855111ll;
    ...
    for(int i=1;i<=n;++i)if(deg[i])++mp[hh[i]^val[i]];
    long long ans=0,deg0=0;
    for(int i=1;i<=n;++i)if(!deg[i])++deg0;
    for(auto it:mp){
        // cout<<it<<endl;
        ans+=it.Y*(it.Y-1)/2;
    }
    mp.clear();
    for(int i=1;i<=n;++i)if(deg[i])++mp[hh[i]];
    for(auto it:mp){
        // cout<<it<<endl;
        ans+=it.Y*(it.Y-1)/2;
    }
    cout<<ans+deg0*(deg0-1)/2<<endl;
}
