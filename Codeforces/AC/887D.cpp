...
struct event{ll t,type,rating;};

event deq[300005];
ll pre,exitpre; 
int front=0,back=-1;
deque<int> dq;
void addShowEvent(event e){
    PDE("add");
    // add a event to end
    deq[++back]=e;
    deq[back].rating=(deq[back].type?c:-d);
    deq[back].rating+=pre;
    pre=deq[back].rating;
    while(dq.size() && deq[dq.back()].rating>=deq[back].rating)dq.pop_back();
    dq.push_back(back);
}
bool checkOK(ll now){
    // just check elements in show
    return now+deq[dq.front()].rating-exitpre>=0;
}
event getFirst(){
    // if nothing, return {-1,-1}
    if(front>back)return {-1,-1};
    return deq[front];
}
void eraseShowEvent(){
    PDE("erase");
    exitpre+=(deq[front].type?c:-d);
    PDE(exitpre);
    ++front;
}
...
int main(){
    ...
    ll now=start,nowT=0;
    while(es.size() && es.front().t<nowT+len){
        addShowEvent(es.front());
        es.pop_front();
    }
    if(checkOK(now)){
        cout<<nowT<<endl;
