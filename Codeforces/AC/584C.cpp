...
int main(){
    ...
    string a,b; cin>>a>>b;
    int same=0,usame=0;
    for(int i=0;i<n;++i)++(a[i]==b[i]?same:usame);
    k=n-k;
    int mmmm=k-same; mmmm=max(mmmm,0);
    int ma=mmmm,mb=mmmm;
    string ans;
    for(int i=0;i<n;++i){
        if(k && a[i]==b[i]){
            ...
            if(a[i]!='a' && b[i]!='a')ans+="a";
            else if(a[i]!='b' && b[i]!='b')ans+="b";
            else if(a[i]!='c' && b[i]!='c')ans+="c";
        }
    }
    if(ma>0 || mb>0)cout<<-1<<endl;
    else cout<<ans<<endl;
}
