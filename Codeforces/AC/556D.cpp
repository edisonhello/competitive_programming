...
int main(){
    ...
    sort(seg,seg+n-1,[](const pair<pll,int> &a,const pair<pll,int> &b){return a.X.Y==b.X.Y?a.X.X<b.X.X:a.X.Y<b.X.Y;});
    set<pair<ll,int>> st;
    for(int i=0;i<m;++i)st.insert(b[i]);
    for(int i=0;i<n-1;++i){
        auto it=st.lower_bound({seg[i].X.X,-233});
        if(it==st.end() || it->X>seg[i].X.Y)cout<<"No"<<endl,exit(0);
        ans[seg[i].Y]=it->Y; st.erase(it);
    }
