...
void yo(){
    cout<<ans.size()<<endl;
    for(pii i:ans)cout<<i.X<<" "<<i.Y<<endl;
    exit(0);
}
int main(){
    int n; cin>>n;
    ...
    while(q.size()){
        if(pq.empty())yo();
        ans.push_back({q.front().Y,pq.top().Y});
        if(pq.top().X>0)q.push(pq.top()); pq.pop();
        if(q.front().X>1)q.push({q.front().X-1,q.front().Y});
        q.pop();
    }
    if(ans.size()!=n-1)JIZZ();
    yo();
}
