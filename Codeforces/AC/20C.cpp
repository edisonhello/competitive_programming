...
void dij(){
    for(int i=2;i<100005;++i)d[i]=999999999999999ll;
    pq.push({0,1});
    while(pq.size()){
        ...
        int np=pq.top().Y;
        for(pii i:G[np]){
            if(!v[i.X] && i.Y+d[np]<d[i.X]){
                d[i.X]=i.Y+d[np]; p[i.X]=np;
                pq.push({d[i.X],i.X});
            }
        }
        // cout<<"d: ";for(int i=1;i<=5;++i)cout<<d[i]<<" ";el;
    }
}
main(){
    int n,m;cin>>n>>m;
