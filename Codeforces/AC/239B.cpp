...
void solve(string s){
    int ptr=0,dx=1;
    int cnt[10]={0};
    while(ptr>=0 && ptr<s.size()){
        PDE2(ptr,dx);
        if(isdig(s[ptr]))++cnt[s[ptr]-'0'],s[ptr]--;
        else s[ptr]=='>'?dx=1:dx=-1;
        int prv=ptr;
        PDE1(s[ptr]);
        do ptr+=dx;while(ptr>=0 && ptr<s.size() && s[ptr]=='0'-1);
        PDE3(prv,ptr,dx);
        if(ptr<0 || ptr>=s.size())break;
        if(!isdig(s[prv]) && !isdig(s[ptr]))s[prv]='0'-1;
    }
