...
int main(){
    ...
    for(int i=2;i<=n;++i)pre[i]=x[i]+pre[i-1];
    for(int i=2;i<=n;++i)dp[i][1]=x[i]*i-pre[i];
    for(int i=2;i<=n;++i){
    ...
    ll mn=(1ll<<58);
    for(int i=k;i<=n;++i)mn=min(mn,dp[i][k]+(pre[n]-pre[i])-x[i]*(n-i));
    cout<<mn<<endl;
    if(DEBUG){
        cout<<"----------"<<endl;
        for(int i=1;i<=n;++i){
