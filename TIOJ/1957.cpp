...
vint lisan;
int lisans[200009],lisant[200009];
ll plan_roller_coaster(int n,int *s,int *t){
    for(int i=0;i<n;++i){
        lisan.pb(s[i]), lisan.pb(t[i]);
    }
    lisan.resize(unique(lisan.begin(),lisan.end())-lisan.begin());
    for(int i=0;i<n;++i){
        lisans[i]=lower_bound(lisan.begin(),lisan.end(),s[i])-lisan.begin();
        lisant[i]=lower_bound(lisan.begin(),lisan.end(),t[i])-lisan.begin();
    }
}
