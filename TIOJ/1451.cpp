...
bitset<100005> b;
int main(){
    int n,m; cin>>n>>m; for(int i=1;i<=n;++i)d[i]=i; while(m--){
        int a,b; cin>>a>>b; d[b]=a;
    } int cnt=0; for(int i=1,u;i<=n;++i){
        PDE2(i,b[i]);
        if(b[i])continue;
        for(u=d[i];d[u]!=u && u!=i;u=d[u])if(b[u])break;
        PDE2(u,b[u]);
        if(b[u])continue; ++cnt;
        for(u=i;!b[u];u=d[u])b[u]=1; b[u]=1;
    } cout<<cnt<<endl;
}
