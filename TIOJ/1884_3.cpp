...
int main(){
    ...
    while(t--){
        ...
        for(int i=0;i<n;i++){
            dis.at(i) = min(wait.at(n) , l-wait.at(n));

            if(i-k>=0)fnt.at(i) = fnt.at(i-k)+dis.at(i)*2;
            else fnt.at(i) = dis.at(i)*2;
        }
        for(int i=n-1;i>=0;i--){
            if(i+k<=n)bak.at(i)=bak.at(i+k)+dis.at(i)*2;
            else bak.at(i)=dis.at(i)*2;
        }

        ll findmid=0;
        while(wait.at(findmid++)<=l/2) continue;

        ll cas1,cas2 = 1000000000;
        cas1 = fnt.at(findmid) + bak.at(n-findmid);
        for(int i=0;i<n;i++){
            cas2 = min(cas2, fnt.at(findmid-i)+bak.at(findmid+i-k));
        }
