...
int main(){
    ...
    for(int i=1;i<=n;++i){
        ...
        for(int j=1;j<=m;++j){
            PDE1(st);
            while(st.size() && mp[i][st.top()]>=mp[i][j])st.pop();
            if(st.empty())st.push(0);
            mx=max(mx,min((int)mp[i][j],st.top()?mp[i][st.top()]:j-st.top()));
            tot+=min((int)mp[i][j],st.top()?mp[i][st.top()]:j-st.top());
            PDE3(i,j,min((int)mp[i][j],st.top()?mp[i][st.top()]:j-st.top()));
            st.push(j);
        }
    }
    cout<<tot<<" "<<mx<<endl;
