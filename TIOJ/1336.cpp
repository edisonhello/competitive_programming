...
string mp[111];
bool vg[111][111],vd[111][111];

void dfsG(int x,int y){
    vg[x][y]=1;
    if(mp[x+1][y-1]=='G' && !vg[x+1][y-1]){dfsG(x+1,y-1);}
    if(mp[x+1][y]=='G' && !vg[x+1][y]){dfsG(x+1,y);}
    if(mp[x+1][y+1]=='G' && !vg[x+1][y+1]){dfsG(x+1,y+1);}
    if(mp[x][y+1]=='G' && !vg[x][y+1]){dfsG(x,y+1);}
    if(mp[x-1][y+1]=='G' && !vg[x-1][y+1]){dfsG(x-1,y+1);}
    if(mp[x-1][y]=='G' && !vg[x-1][y]){dfsG(x-1,y);}
    if(mp[x-1][y-1]=='G' && !vg[x-1][y-1]){dfsG(x-1,y-1);}
    if(mp[x][y-1]=='G' && !vg[x][y-1]){dfsG(x,y-1);}
...
void dfsD(int x,int y){
    vd[x][y]=1;
    if(mp[x+1][y-1]=='-' && !vd[x+1][y-1]){dfsD(x+1,y-1);}
    if(mp[x+1][y]=='-' && !vd[x+1][y]){dfsD(x+1,y);}
    if(mp[x+1][y+1]=='-' && !vd[x+1][y+1]){dfsD(x+1,y+1);}
    if(mp[x][y+1]=='-' && !vd[x][y+1]){dfsD(x,y+1);}
    if(mp[x-1][y+1]=='-' && !vd[x-1][y+1]){dfsD(x-1,y+1);}
    if(mp[x-1][y]=='-' && !vd[x-1][y]){dfsD(x-1,y);}
    if(mp[x-1][y-1]=='-' && !vd[x-1][y-1]){dfsD(x-1,y-1);}
    if(mp[x][y-1]=='-' && !vd[x][y-1]){dfsD(x,y-1);}
...
int main(){
    ...
    CIO; int w,h,i,j,g=0,d=0; cin>>w>>h;
    for(i=1;i<=h;++i){
        cin>>mp[i];
        mp[i]="B"+mp[i]+"B";
    }
    for(i=0;i<=w+1;++i)mp[0]+="B";mp[h+1]+="B";
    for(i=1;i<=h;++i){
        for(int j=1;j<=w;++j){
            if(mp[i][j]=='G' && !vg[i][j])dfsG(i,j),++g;
            if(mp[i][j]=='-' && !vd[i][j])dfsD(i,j),++d;
        }
    }
    cout<<g<<" "<<d<<endl;
}
