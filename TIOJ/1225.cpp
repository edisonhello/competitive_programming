...
int a[1000009],cst[1000009];

int main(){
    int n;cin>>n;
    for(int i=0;i<n;++i)cin>>a[i],cst[i]=1111111111;
    stack<int> st;
    for(int i=0;i<n;++i){
        while(st.size() && st.top()<a[i])st.pop();
        if(st.size())cst[i]=st.top();
        st.push(a[i]);
    }
    while(st.size())st.pop();
    for(int i=n-1;i>=0;--i){
        while(st.size() && st.top()<a[i])st.pop();
        if(st.size())cst[i]=min(cst[i],st.top());
        st.push(a[i]);
    }
    for(int i=0;i<n;++i)cout<<cst[i]<<" ";el;
    ll ans=0;
    for(int i=0;i<n;++i){
        if(cst[i]==1111111111)continue;
        ans+=cst[i];
    }
    cout<<ans<<endl;
}
