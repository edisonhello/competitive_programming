...
int main(){
    int ts; cin>>ts; while(ts--){
        int n; cin>>n; vint v; while(n--){
            int t; cin>>t; v.pb(t);
        }
        for(int ptr=v.size()-2,off;ptr>=0;--ptr){
            off=1;
            while(ptr+off<v.size() && (v[ptr+off]==0 || v[ptr]%v[ptr+off]==0)){
                if(v[ptr+off]){
                    v[ptr]/=v[ptr+off];
                    v[ptr+off]=0;
                }
        ...
        if(jizz)cout<<"zzz..."<<endl;
        else cout<<"Asssss!!"<<endl;
    }
}
