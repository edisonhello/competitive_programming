...
int oppo[30009];
char mem[30009];
void imple(int len,string input,unsigned int ipptr=0){
    MS0(mem); char *ptr=mem;
    for(int i=0;i<len;++i){
        switch(code[i]){
            case '>': ++ptr; break;
            case '<': --ptr; break;
            ...
            case ',': *ptr=ipptr<input.size()?input[ipptr++]:-1; break;
            case '[': if((*ptr)==0)i=oppo[i]; break;
            case ']': if(*ptr)i=oppo[i]; break;
        }
        // PDE3(i,code[i],ptr-mem);
    }
}
        ...
        for(char &c:s){
            switch(c){
                case '>': case '<': case '+': case '-': case '.': case ',': case '[': case ']':
                    code[len++]=c;
                    // if(DEBUG)printf("%c",code[len-1]);
            }
        }
    }
    stack<int> pos;
    for(int i=0;i<len;++i){
        if(code[i]=='[')pos.push(i);
        else if(code[i]==']')oppo[pos.top()]=i, oppo[i]=pos.top(), pos.pop();
    }
    int op; cin>>op; getline(cin,s);
    while(op--){
        getline(cin,s);
        s+=(char)10;
        imple(len,s);
