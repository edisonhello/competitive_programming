...
int main(){
    ...
    deque<int> dq; int now=0,mx=1<<31,plz;
    for(int i=0;i<n;++i){
        while(dq.size() && i-dq.front()>=k)now-=b[dq.front()],dq.pop_front();
        while(dq.size() && h[dq.back()]<=h[i])now-=b[dq.back()],dq.pop_back();
        now+=b[i]; dq.push_back(i);
        if(now>mx)mx=now,plz=i;
    }
    cout<<plz+1<<" "<<mx<<endl;
}
