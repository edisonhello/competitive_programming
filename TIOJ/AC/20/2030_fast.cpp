...
void cdq(int l,int r){
    ...
    int mid=(l+r)>>1;
    for(int i=l;i<=mid;++i){
        if(ev[i].type==1)pro.push_back(i);
    }
    for(int i=mid+1;i<=r;++i){
        if(ev[i].type==2)pro.push_back(i);
    }
    sort(pro.begin(),pro.end(),[](const int &a,const int &b){return ev[a].r==ev[b].r?ev[a].l==ev[b].l?ev[a].type<ev[b].type:ev[a].l>ev[b].l:ev[a].r<ev[b].r;});
    int cnt=0;
    for(auto &i:pro){
        if(ev[i].type==1){
            add(ev[i].l,ev[i].v);
            ...
            ans[ev[i].v]+=query(ev[i].l);
        }
    }
    if(cnt>=5000)memset(bit,0,sizeof(bit));
    else for(auto &i:pro)if(ev[i].type==1)add(ev[i].l,-ev[i].v);
