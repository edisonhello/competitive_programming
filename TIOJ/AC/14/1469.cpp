...
int cf[150],bn[150];
bitset<150> inq;
bool floww(int st,int ed,int &nans){
    MS0(bn); bn[st]=88877;
    inq.reset(); inq[st]=1;
    ...
    while(q.size()){
        ...
        for(int eid:G[np]){
            if(bn[edg[eid].v]<min(bn[np],edg[eid].f)){
...
int main(){
    int n,m; while(cin>>n>>m){
        ...
        n/=6;
        for(int i=1;i<=6;++i)ae(0,i,n);
        for(int i=0;i<m;++i){
            string s,ss; cin>>s>>ss; 
            int a=gid(s),b=gid(ss);
            ae(a,i+10,1);
            ae(b,i+10,1);
            ae(i+10,149,1);
        }
        if(flow(0,149)==m)cout<<"YES"<<endl;
        else cout<<"NO"<<endl;
    }
}
