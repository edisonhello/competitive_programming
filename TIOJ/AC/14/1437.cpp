...
int w[156][156],mx[156];
set<pii> ans;
int main(){
    int n; cin>>n;
    for(int i=1;i<=n;++i)for(int j=1;j<=n;++j)cin>>w[i][j];
    for(int i=1;i<=n;++i)ans.insert(pii(1,i));
    for(int i=2;i<=n;++i){
        for(int j=1;j<=n;++j)mx[i-1]=max(mx[i-1],w[i-1][j]);
        for(int j=1;j<=n;++j)if(w[i][j]>mx[i-1])ans.insert(pii(i,j));
    }
    for(auto i:ans)cout<<"("<<i.Y-1<<","<<i.X-1<<")"<<endl;
}
