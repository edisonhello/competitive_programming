...
ll calc(int l,int r){
    ...
    for(int i=l+1;i<=r;++i){
        dp[l][r]=min(dp[l][r],calc(l,i-1)+calc(i,r)+itm[l].r*itm[i-1].c*itm[r].c);
    }
    return dp[l][r];
}
int main(){
    ...
    int ts; cin>>ts; while(ts--){
        MS0(itm); MS0(u);
        int n; cin>>n;
        for(int i=1;i<=n;++i)cin>>itm[i].r>>itm[i].c;
        ans.pb(calc(1,n));
    }
    for(ll i:ans)tot+=i;
    // for(ll i:ans)cout<<i<<endl;
    for(ll i:ans)cout<<(i-1)/1000+1<<endl;
    cout<<(tot-1)/1000+1<<endl;
}
