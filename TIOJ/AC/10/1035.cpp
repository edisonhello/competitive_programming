...
char s[50009],t[50009];
int main(){
    gets(s), gets(t);
    int sl=strlen(s), tl=strlen(t),mx=0;
    for(int i=0,cnt,j;i<tl;++i){
        cnt=j=0;
        for(;i+j<sl;++j)if(s[i+j]==t[j])++cnt;
        mx=max(mx,cnt);
    }
    for(int i=0,cnt,j;i<sl;++i){
        cnt=j=0;
        for(;i+j<tl;++j)if(t[i+j]==s[j])++cnt;
        mx=max(mx,cnt);
    }
    pit(mx),el;
}
