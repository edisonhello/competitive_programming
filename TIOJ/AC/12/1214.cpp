...
int main(){
    int n;while(cin>>n,n){
        Gs.clear(), Hs.clear(); Gs.resize(n), Hs.resize(n);
        for(int i=0;i<111;++i)G[i].clear(), H[i].clear();
        for(int i=1;i<n;++i){
            int u,v;cin>>u>>v;--u,--v;
            G[u].pb(v), G[v].pb(u);
        }
        ...
        for(int root=0;root<n;++root){
            ...
            PDE2(root,Hs);
            for(int i=0;i<n;++i)if(Gs[i]!=Hs[i])goto nextRoot;
            cout<<"Same\n";goto nextTest;
            nextRoot:;
        }
        cout<<"Different\n";
