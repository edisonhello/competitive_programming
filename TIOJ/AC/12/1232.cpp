...
int main(){
    ...
    cin>>n>>r;
    vint perm;
    for(int i=r;i<n;++i)perm.pb(0);
    for(int i=0;i<r;++i)perm.pb(1);
    do{
        vint tn;
        for(int i=0;i<n;++i)if(perm[i])tn.pb(a[i]);
        sort(tn.begin(),tn.end());
        do{
            ll t=0;
            for(int i:tn)t=t*10+i;
            nums.pb(t);
        }while(next_permutation(tn.begin(),tn.end()));
    }while(next_permutation(perm.begin(),perm.end()));
    sort(nums.begin(),nums.end());
    if(n+r>nums.size())cout<<0<<endl;
    else cout<<nums[nums.size()-n-r]<<endl;
}
