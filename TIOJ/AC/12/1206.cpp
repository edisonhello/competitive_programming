...
ll hsh[200009],pp[200009];

int main(){
    pp[0]=1; for(int i=1;i<200005;++i)pp[i]=pp[i-1]*p%mod;
    string s; while(cin>>s){
        hsh[0]=s[0];
        for(auto i=1u;i<s.length();++i){
            hsh[i]=hsh[i-1]+s[i]*pp[i]; hsh[i]%=mod;
        }
        int mxl=(int)s.length()/2;
        for(;mxl>=0;--mxl){
            ll first=((hsh[s.length()-1-mxl]-hsh[s.length()-1-2*mxl])%mod+mod)%mod;
            ll second=((hsh[s.length()-1]-hsh[s.length()-1-mxl])%mod+mod)%mod;
            if(first*pp[mxl]%mod==second){
                cout<<mxl<<endl;
                break;
            }
        }
