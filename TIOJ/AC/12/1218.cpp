...
bool all0(string s){
    for(char &c:s)if(c!='0')return 0;return 1;
}
int main(){
    string s;while(cin>>s){
        if(all0(s))exit(0);
        if((s[s.length()-1]-'0')&1)cout<<"LOSE ";
        else cout<<"WIN ";
        if(all0(s.substr(0,s.length()-1))){
            int dig=s[s.length()-1]-'0';
            if(dig==1 || dig==3)cout<<"WIN\n";
            else if(dig==2)cout<<"LOSE\n";
            else if(dig&1)cout<<"LOSE\n";
            else cout<<"WIN\n";
        }
        else if((s[s.length()-1]-'0')&1)cout<<"LOSE\n";
        else cout<<"WIN\n";
    }
}
