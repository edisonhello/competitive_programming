...
int ino[12345],idx[12345],n;

void dfs(int l,int r,int ptr){
    if(l>r)return;
    for(;ptr<n;++ptr){
        if(idx[ptr]>=l && idx[ptr]<=r){
            pit(ino[idx[ptr]]),spc;
            dfs(l,idx[ptr]-1,ptr); dfs(idx[ptr]+1,r,ptr);
            break;
        }
    }
}
...
int main(){
    while(rit(n),n){
        for(int i=0;i<n;++i)rit(ino[i]),idx[i]=i;
        sort(idx,idx+n,[](const int &a,const int &b)->bool{return ino[a]<ino[b];});
        dfs(0,n-1,0); el;
    }
}
