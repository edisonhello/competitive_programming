...
struct cm{char t;int x,y;} history[550009];
...
void insert(node *&root,int v){
    ...
    root=merge(merge(merge(a,b),new node(v)),c);
    LOG("finish merge\n");
}
void change(int whr,int hmn){
    ...
    root[F(whr)]=merge(merge(merge(a,b->l),b->r),c); delete b;
...
void read(int &n,int &m,int &q){
    for(int i=0;i<n;++i)rit(people[i]);
    for(int i=0;i<m;++i)rit(edge[i].u,edge[i].v);
    LOG("reading...\n");
    for(;;++q){ history[q].t=getchar();
        if(history[q].t=='Q')rit(history[q].x,history[q].y);
        else if(history[q].t=='C'){ rit(history[q].x,history[q].y);
            swap(people[history[q].x],history[q].y);
        }
        else if(history[q].t=='D'){ rit(history[q].x);
