...
int32_t main(){
    ...
    auto meow=[&](function<void()> callback){
        // memset(pre,0xb0,sizeof(pre));
        memset(rans,0xb0,sizeof(rans));
        memset(lans,0xb0,sizeof(lans));
        memset(uans,0xb0,sizeof(uans));
        memset(dans,0xb0,sizeof(dans));

        auto get=[&](int x,int y){
            if(x<k || y<k)return 0l;
