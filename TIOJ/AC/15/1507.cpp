...
vint times(vint a,vint b){
    vint rt(a.size()+b.size(),0);
    for(int i=0;i<a.size();++i){
        for(int j=0;j<b.size();++j){
            rt[i+j+1]+=a[i]*b[j];
        }
    }
    for(int i=rt.size()-1;i;--i){
        rt[i-1]+=rt[i]/10;
        rt[i]%=10;
    }
    while(rt[0]>10){
        vint meow; meow.pb(rt[0]/10); rt[0]%=10;
        for(int i:rt)meow.pb(i); rt=meow;
    }
    while(rt.size() && rt[0]==0){
        vint meow;
        for(int i=1;i<rt.size();++i)meow.pb(rt[i]);
        rt=meow;
    }
    return rt;
}
...
char sa[55],op[3],sb[55];
int main(){
    scanf("%s%s%s",sa,op,sb);
    for(int i=0;sa[i]!='\0';++i)a.pb(sa[i]-'0');
    for(int i=0;sb[i]!='\0';++i)b.pb(sb[i]-'0');
    vint ans;
    if(op[0]=='/')ans=div(a,b);
    else ans=times(a,b);
    if(ans.size()==0)ans.pb(0);
    for(auto &c:ans)putchar(c+'0');
}
