...
void count2(int n){
    ...
    while(n--){
        ...
        for(int i=0;i<v.size()-1;++i)if(v[i]>99999)v[i]-=100000,v[i+1]++;
        if(v[v.size()-1]>99999)v[v.size()-1]-=100000,v.pb(1);
    }
    reverse(v.begin(),v.end());
    cout<<v[0];
    for(int i=1;i<v.size();++i)printf("%05d",v[i]); cout<<'\n';
}
int main(){
    int n; cin>>n;
    count2(n-1);
    ...
    PDE1(h[0]);
    stack<int> s; s.push(h[0].Y);
    queue<int> q;
    for(int i=1;i<n;++i){
        if(h[i].Y<s.top())s.push(h[i].Y);
        else q.push(h[i].Y);
    }
