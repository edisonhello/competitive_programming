...
int main(){
    ld x1,y1,x2,y2; while(cin>>x1>>y1>>x2>>y2){
        ld xm=(x1+x2)/2,ym=(y1+y2)/2;
        ld xd=(x2-x1)*sqrt(3.)/2,yd=(y2-y1)*sqrt(3.)/2;
        vector<pair<ld,ld>> pos;
        pos.eb(xm+yd,ym-xd);
        pos.eb(xm-yd,ym+xd);
        sort(pos.begin(),pos.end());
        cout<<fixed<<setprecision(3)<<pos[0].X<<" "<<pos[0].Y<<" "<<pos[1].X<<" "<<pos[1].Y<<endl;
    }
}
