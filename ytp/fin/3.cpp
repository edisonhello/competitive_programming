...
int main(){
    ...
    for(char c:s) if(c=='Z') JIZZ();
    PDE1(s);
    int tot=0;
    for(int i=s.length()-1,base=1;i>=0;--i,base*=25){
        tot+=(s[i]-'A'+1)*base;
    }
    stack<char> an;
    PDE1(tot);
    while(tot){
        int dig=tot%16;
        PDE1(dig);
        if(dig<10)an.push((char)(dig+'0'));
        else an.push((char)(dig-10+'A'));
        tot/=16;
    }
    while(an.size()<6u)an.push('0');
    while(an.size()){
        cout<<an.top();
        an.pop();
    }
