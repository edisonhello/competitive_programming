...
int main(){
    CPPinput;
    sol.setN(205);
    for(int i=1;i<=100;++i)sol.addEdge(0,i,1,0);
    for(int i=1;i<=100;++i)sol.addEdge(i+100,204,1,0);
    for(int i=1;i<=100;++i)for(int j=1;j<=100;++j)mp[i][j+100]=mp[j+100][i]=0;
    string s; while(getline(cin,s)){
        if(s[0]!='M' && s[0]!='F')break;
        SS ss(s);
        string pp; ss>>pp;
        int sex=pp[0];
        SS sss(s.substr(1,s.size()-1));
        int num; sss>>num;
        int id=num+(sex=='F'?100:0);
        string s; while(ss>>s){
            PDE(s);
            int to,val;
            int pos=s.find(',');
            SS ssss(s.substr(0,pos)),sssss(s.substr(pos+1,s.size()-pos-1));
            ssss>>to; sssss>>val;
            mp[id][to+(sex=='F'?0:100)]=val;
            PDE(id,to,val);
        }
    }
    for(int i=1;i<=100;++i){
        for(int j=1;j<=100;++j){
            int v=min(mp[i][j+100],mp[j+100][i]);
            sol.addEdge(i,j+100,1,-v+100);
        }
    }
    auto res=sol.flow(0,204);
    PDE(res);
    cout<<10000-res.second<<endl;
}
