...
int main(int argc, char **argv) {
	...
	vector<tuple<int, MyInputData, OutputData>> pool;
	pool.emplace_back(score, in_data, out_data);
	
	while (true) {
		cout << "pts ";
		for (auto &t : pool) cout << get<0>(t) << ' ';
		cout << endl;

		vector<tuple<int, MyInputData, OutputData>> new_pool = pool;
		set<int> score_set;
		for (auto &[oc, id, od] : pool) {
			for (int t = 0; t < 30; ++t) {
				for (int i = 0; i < 30; ++i) {
					auto new_in_data = id;
					new_in_data.Swap();
					...
					if (new_score > oc && score_set.count(new_score) == 0) {
						new_pool.emplace_back(new_score, new_in_data, new_out_data);
		...
		sort(new_pool.begin(), new_pool.end(), [](const auto &a, const auto &b) {
	  return get<0>(a) > get<0>(b);
				});

		if (new_pool.size() > 30u) new_pool.resize(30);

		cout << "new sc " << get<0>(new_pool[0]) << endl;
		if (get<0>(new_pool[0]) > score) {
			score = get<0>(new_pool[0]);
			Output(get<1>(new_pool[0]), get<2>(new_pool[0]), get<0>(new_pool[0]));
		}
