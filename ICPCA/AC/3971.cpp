...
bool chk(int mq,int bg){
    for(int i=1;i<=_sz;++i){
        int mnc=1000000009;
        for(auto pi:v[i]){
            if(pi.Y>=mq)mnc=min(mnc,pi.X);
        }
        bg-=mnc;
        if(bg<0)return 0;
    } return 1;
...
int main(){
    int ts;cin>>ts;while(ts--){
        for(int i=1;i<=_sz;++i)v[i].clear(); _sz=0; mp.clear();

        int n,bg;cin>>n>>bg;
        for(int i=0;i<n;++i){
            string s,t;int a,b;cin>>s>>t>>a>>b;
            int id=gid(s); v[id].push_back({a,b});
        }
        int L=0,R=1e9;
        while(R>L){
            int M=(L+R)>>1;
        ...
        if(chk(L,bg))cout<<L<<endl;
        else if(chk(L-1,bg))cout<<L-1<<endl;
        else assert(1==0);
    }
}
