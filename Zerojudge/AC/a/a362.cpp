...
struct it{int h,w,i;} a[10005];
int main(){
    int n; while(cin>>n){
        for(int i=0;i<n;++i){
            cin>>a[i].h>>a[i].w; a[i].i=i;
        }
        sort(a,a+n,[](const it &a,const it &b){return a.h==b.h?a.w==b.w?a.i<b.i:a.w<b.w:a.h<b.h;});
        int tot=0;
        for(int i=0;i<n;++i){
            tot+=abs(a[i].i-i);
            PDE(i,tot);
            PDE(a[i].h,a[i].w,a[i].i);
        }
        cout<<tot<<endl<<endl;
    }
}
