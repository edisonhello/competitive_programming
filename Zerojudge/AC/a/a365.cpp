...
bool sg(int x){
    ...
    if(x==0)return 1;
    v[x]=1;
    for(int i=0;i<12;++i)if(x&(1<<i))_sg[x]|=!sg(x^(1<<i));
    for(auto &i:tkp)if((x&(1<<i.X))&&(x&(1<<i.Y)))_sg[x]|=!sg(x^(1<<i.X)^(1<<i.Y));
    return _sg[x];
}

#define _(a,b) tkp.emplace_back(a-1,b-1);
int main(){
    _(1,4); _(2,5); _(4,8); _(5,9); _(8,11); _(9,12);
    _(3,4); _(4,5); _(5,6); _(7,8); _(8,9); _(9,10);
    int n; cin>>n; while(n--){
        string s; cin>>s; int x=0;
        for(int i=0;i<12;++i)if(s[i]=='1')x|=(1<<i);
        cout<<sg(x);
    }
}
