...
pair<bool,int> ju[22][2];
bool ff[22];
int main(){
    CIO; cin.tie(0);
    int ts;while(cin>>ts){while(ts--){
        memset(ju,0,sizeof(ju));
        int n,m;cin>>n>>m;
        for(int i=0;i<m;++i){
            char c; int tmp;
            cin>>c>>tmp;ju[i][1]={c=='m',tmp};
            cin>>c>>tmp;ju[i][0]={c=='m',tmp};
        }
        for(int i=0;i<(1<<n);++i){
            MS0(ff);
            for(int j=0;j<n;++j){
                for(int k=0;k<m;++k){
                    if(ff[k])continue;
                    if(ju[k][0].X==!!(i&(1<<j)) && ju[k][0].Y==j+1)ff[k]=1;
                    else if(ju[k][1].X==!!(i&(1<<j)) && ju[k][1].Y==j+1)ff[k]=1;
                }
            }
            for(int j=0;j<m;++j)if(!ff[j])goto nexT;
            cout<<"GOOD\n";
            goto neXT;
            nexT:;
        }
