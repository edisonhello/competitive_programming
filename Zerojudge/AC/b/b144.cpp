...
int n,a[50005],ptr,prv,now,cnt[50005];
pii w[50005];
int main(){
    cin>>n;
    for(int i=1;i<=n;++i)cin>>w[i].X>>w[i].Y;
    a[ptr=1]=1; prv=1; now=w[1].X;
    while(ptr<=n){
        if(w[now].X!=prv && w[now].Y!=prv)JIZZ();
        if(now==1 && ptr<n)JIZZ();
        a[++ptr]=prv=now;
        now=(w[now].X==a[ptr-1]?w[now].Y:w[now].X);
    }
    // for(int i=1;i<=n;++i)cout<<a[i]<<" ";cout<<endli;
    int mx=0;
    for(int i=1;i<=n;++i)++cnt[(a[i]+n-i)%n];
    for(int i=0;i<=n;++i)mx=max(mx,cnt[i]);
    MS0(cnt); reverse(a+2,a+n+1);
    for(int i=1;i<=n;++i)++cnt[(a[i]+n-i)%n];
    for(int i=0;i<=n;++i)mx=max(mx,cnt[i]);
    cout<<n-mx<<endl;
}
