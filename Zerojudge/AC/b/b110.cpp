...
string mp[1<<8];
int __buffer__[1<<15],pre[1<<8][1<<8];
vector<char> ans;

void meow(int x1,int y1,int x2,int y2,int l){
    PDE5(x1,y1,x2,y2,l);
    // PDE5(pre[x2][y2],pre[x1-1][y2],pre[x2][y1-1],pre[x1-1][y1-1],pre[x2][y2]-pre[x1-1][y2]-pre[x2][y1-1]+pre[x1-1][y1-1]);
    if(pre[x2][y2]-pre[x1-1][y2]-pre[x2][y1-1]+pre[x1-1][y1-1]==0){
        ans.pb('w');
        return;
    }
    if(pre[x2][y2]-pre[x1-1][y2]-pre[x2][y1-1]+pre[x1-1][y1-1]==l*l){
        ans.pb('b');
        return;
    }
    ans.pb('g');
    int xm=(x1+x2)>>1, ym=(y1+y2)>>1;
    meow(x1,ym+1,xm,y2,l>>1);
    meow(x1,y1,xm,ym,l>>1);
    meow(xm+1,y1,x2,ym,l>>1);
    meow(xm+1,ym+1,x2,y2,l>>1);
}
int main(){
    int n; while(cin>>n){
    for(int i=0;i<(1<<8);++i)mp[i]="";
    ans.clear(); memset(pre,0,sizeof(pre));
    for(int i=0;i<n;++i){
        cin>>mp[i];
        for(int j=0;j<n;++j){
