...
int dfs(int now){
    if(G[now].empty())return 1;
    int tk=(int)G[now].size();tk*=perc;--tk;tk/=100;++tk;
    PQ<int,vector<int>,less<int>> pq;
    for(int i:G[now]){
        pq.push(dfs(i));
        if(pq.size()>tk)pq.pop();
    }
    int tot=0;while(pq.size()){tot+=pq.top();pq.pop();}
    return tot;
}

int main(){
    int n;while(cin>>n>>perc,n){
        for(int i=0;i<=n;++i)G[i].clear();
        for(int i=1;i<=n;++i){
            int pa;cin>>pa;G[pa].pb(i);
        }
        cout<<dfs(0)<<endl;
    }
}
