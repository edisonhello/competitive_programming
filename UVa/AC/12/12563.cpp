...
int main(){
    ...
    int ts,ks=0;cin>>ts;while(ts--){
        int n,t;cin>>n>>t;int tott=0;
        memset(dp,0,sizeof(dp));
        for(int i=0;i<n;++i)cin>>a[i],tott+=a[i];
        int mxv=1,edt=0;
        dp[0]=1;
        for(int i=0;i<n;++i){
            for(int j=min(tott,t-1);j>=a[i];--j){
                dp[j]=max(dp[j],dp[j-a[i]]+1);
                if(dp[j]>mxv)mxv=dp[j],edt=j;
                if(dp[j]==mxv&&edt<j)edt=j;
            }
        }
        cout<<"Case "<<(++ks)<<": "<<mxv<<" "<<edt+678<<endl;
    }
}
