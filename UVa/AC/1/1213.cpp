...
void init(){
    ...
    for(int i=2;i*i<1123;++i){
        if(np[i])continue;
        for(int j=i*i;j<1123;j+=i)np[j]=1;
    }
    for(int i=2;i<1123;++i)if(!np[i])p.pb(i);
}

int dfs(int n,int k,int u){
    // PDE3(n,k,u);FLH;
    if(k<0 || n<0 || u<0)return 0;
    if(!n && !k)return 1;
    if(v[n][k][u])return dp[n][k][u];
    v[n][k][u]=1;
    return dp[n][k][u]=dfs(n-p[u-1],k-1,u-1)+dfs(n,k,u-1);
}

int main(){
    init();
    int n,k;while(cin>>n>>k,n){
        cout<<dfs(n,k,187)<<endl;
    }
}
