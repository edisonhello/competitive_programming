...
int m_primes[100005],m_bigprime;
vector<int> m_prime_list,ans;

void fj(int m){
    MS0(m_primes); m_bigprime=0; m_prime_list.clear();
    for(int i=2;i*i<=m;++i){
        if(m%i==0) m_prime_list.pb(i);
        else continue;
        while(m%i==0){
            ++m_primes[i];
            m/=i;
        ...
        if(m<=100000) ++m_primes[m],m_prime_list.pb(m);
        else m_bigprime=m;
    } return;
}
int cp(int n,int p,int cnt=0){
    ll np=p;
    while(n>=np){
        cnt+=n/np;
        np*=p;
...
bool check(int k,int n){
    for(int i:m_prime_list){
        int count=cp(n,i)-cp(k,i)-cp(n-k,i);
        if(count<m_primes[i])return 0;
    } return 1;
}
...
int main(){
    int n,m;while(cin>>n>>m && n){
        fj(m);
        if(m_bigprime){cout<<0<<endl<<endl;continue;}
        ans.clear();
        for(int i=1;i<n;++i){
            if(check(i,n-1))ans.pb(i+1);
        }
        cout<<ans.size()<<endl;
