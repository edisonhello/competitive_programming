...
int main(){
    string s;while(cin>>s){
        int cnt00=0,cnt01=0,cnt0=0,cnt1=0;
        for(int i=0;i<s.length()-1;++i){
            if(s[i]=='0'&&s[i+1]=='0')cnt00++;
            if(s[i]=='0'&&s[i+1]=='1')cnt01++;
            if(s[i]=='0')cnt0++;
            else cnt1++;
        } if(s[s.length()-1]=='0')cnt0++;
        else cnt1++;
        if(s[s.length()-1]=='0'){
            if(s[0]=='0')cnt00++;
            else cnt01++;
        }
        ld rot=(ld)cnt0/(ld)(cnt0+cnt1);
        ld sht=(ld)cnt00/(ld)(cnt00+cnt01);
        if(fabs(rot-sht)<eps)cout<<"EQUAL\n";
        else if(rot>sht)cout<<"ROTATE\n";
        else cout<<"SHOOT\n";
    }
}
