...
int main(){
    int ts;cin>>ts;while(ts--){
        ...
        dp[0]=0;
        for(int i=1,d;i<s.length();++i){
            dp[i]=min(dp[i],dp[i-1]+1);
            for(d=1;i+d<s.length()&&i-d>0;++d){
                if(s[i+d]==s[i-d])dp[i+d]=min(dp[i+d],dp[i-d-1]+1);
                else break;
            }
            for(d=0;i+d+1<s.length()&&i-d>0;++d){
                if(s[i+d+1]==s[i-d])dp[i+d+1]=min(dp[i+d+1],dp[i-d-1]+1);
                else break;
            }
        }
