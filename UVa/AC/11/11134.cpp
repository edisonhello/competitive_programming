...
int n;
struct rng{int l,r,i;} xr[5005],yr[5005];
bool operator<(const rng &a,const rng &b){return a.r!=b.r?a.r<b.r:a.l>b.l;}
int x[5005],y[5005];
bool ux[5005],uy[5005];

int main(){
    // FIN;
    while(rit(n),n){
        for(int i=0;i<n;++i)rit(xr[i].l,yr[i].l,xr[i].r,yr[i].r),xr[i].i=i,yr[i].i=i;
        sort(xr,xr+n),sort(yr,yr+n);
        memset(ux,0,sizeof(ux)), memset(uy,0,sizeof(uy));
        bool jizz=0;
        for(int i=0;i<n;++i){
            int xptr=xr[i].l,yptr=yr[i].l;
            while(ux[xptr] && xptr<=xr[i].r)++xptr;
            while(uy[yptr] && yptr<=yr[i].r)++yptr;
            if(xptr>xr[i].r)jizz=1;
            if(yptr>yr[i].r)jizz=1;
            if(jizz)break;
            ux[xptr]=1;x[xr[i].i]=xptr;
            uy[yptr]=1;y[yr[i].i]=yptr;
        }
        if(jizz)puts("IMPOSSIBLE");
        else for(int i=0;i<n;++i)pit(x[i],y[i]),el;
    }
}
