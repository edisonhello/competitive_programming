...
int c(int i,int j){
    return (int)nf[i-1][j]+(int)nf[i][j-1]+(int)nf[i][j+1];
}
int chg(int l){
    if(l>n)return 0;
    int bsch=0;
    for(int i=1;i<=n;++i){
        if(!of[l][i] && nf[l][i])++bsch;
        if(of[l][i] && !nf[l][i])return 98989898;
    }
    for(int i=1;i<=n;++i){
        if(c(l,i)&1) nf[l+1][i]=1;
        else nf[l+1][i]=0;
    } return bsch+chg(l+1);
}
void sol(){
    ...
    for(int p=0;p<(1<<n);++p){
        MS0(nf);
        for(int i=0;i<n;++i)if(p&(1<<i))nf[1][i+1]=1;
        mn=min(mn,chg(1));
    }
    if(mn>99999)cout<<-1<<endl;
    else cout<<mn<<endl;
}
...
int main(){
    int ts,ks=0;cin>>ts;while(ts--){
        cin>>n; MS0(of);
        for(int i=1;i<=n;++i)for(int j=1;j<=n;++j)cin>>of[i][j];
        cout<<"Case "<<(++ks)<<": ";
        sol();
    }
}
