...
vint G[2009],iG[2009],topu;
bool v[2009];

void dfs1(int now){
    v[now]=1;
    for(auto i:G[now])if(!v[i])dfs1(i);
    topu.pb(now);
}
void dfs2(int now){
    v[now]=1;
    for(auto i:iG[now])if(!v[i])dfs2(i);
}

int main(){
    int n,m;while(rit(n,m),n){
        for(int i=1;i<=n;++i)G[i].clear(),iG[i].clear(); topu.clear();
        while(m--){
            int u,v,t;rit(u,v,t);
            if(t==1){
                G[u].pb(v); iG[v].pb(u);
        ...
        bool first=0; for(int i=1;i<=n;++i)if(!v[i]){
            if(first)goto notSCC;
            first=1; dfs1(i);
        }
        for(int i=0;i<=n;++i)v[i]=0;
        first=0; for(int i=n-1;i>=0;--i)if(!v[topu[i]]){
            if(first)goto notSCC;
            first=1; dfs2(topu[i]);
        }
        puts("1");continue;
