...
unsigned ll pw(unsigned ll b,unsigned ll n,unsigned ll m,unsigned ll a=1){
    while(n){
        if(n&1)a=a*b%m;
        b=b*b%m,n>>=1;
    } return a;
...
int main(){
    ...
    int ts;cin>>ts;while(ts--){
        unsigned ll a,b,n,n2,lp=0;cin>>a>>b>>n;n2=n*n;bool fl=0;
        if(n==1ull){cout<<"0"<<endl;continue;}
        F[0]=0,F[1]=1;
        for(int i=2;i<=(int)n2;++i){
            F[i]=(F[i-1]+F[i-2])%n; PDE2(i,F[i]);
            if(F[i]==1ull && fl){
                lp=i-1;break;
            }
            if(F[i]==0ull)fl=1;
