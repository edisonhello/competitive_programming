...
int main(){
    int r,ks=0;while(cin>>n>>r,n){
        for(int i=0;i<n;++i)cin>>och[i]; MS0(ch);
        MS0(bu); for(int i=0;i<r;++i)bu[i]=1; sort(bu,bu+n);
        ld chtot=0.0;
        do{
            ld hpch=getchce(); chtot+=hpch;
            for(int i=0;i<n;++i)if(bu[i])ch[i]+=hpch;
        }while(next_permutation(bu,bu+n));
        cout<<"Case "<<(++ks)<<":\n";
        for(int i=0;i<n;++i)cout<<fixed<<setprecision(6)<<ch[i]/chtot<<endl;
    }
}
