...
vint G[10009],iG[10009],tp;
bool v[10009];
int bel[10009],ind[10009];
...
int main(){
    int ts,ks=0;cin>>ts;while(ts--){
        int n,m;cin>>n>>m;
        for(int i=0;i<=n;++i)G[i].clear(),iG[i].clear(),ind[i]=0; tp.clear();
        while(m--){
            int u,v;cin>>u>>v;
            G[u].pb(v); iG[v].pb(u);
        }
        MS(v,0); for(int i=1;i<=n;++i)if(!v[i])dfs1(i); int sccs=0;
        MS(v,0); for(int i=n-1;i>=0;--i)if(!v[tp[i]])dfs2(tp[i],sccs++);
        for(int i=1;i<=n;++i){
            for(int ii:G[i]){
