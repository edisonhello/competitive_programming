...
int main(){
    int ts;cin>>ts;while(ts--){
        int n;cin>>n; MS0(id), MS0(od), MS0(us);
        for(int i=0;i<33;++i)G[i].clear();
        for(int i=0;i<n;++i){
            string s;cin>>s; int l=s.length();
            G[s[0]-'a'].push_back({s[l-1]-'a',i});
            ++od[s[0]-'a'], ++id[s[l-1]-'a'];
        }
        // cout<<"id: ";for(int i=0;i<26;++i)cout<<id[i]<<" ";cout<<endl;
        // cout<<"od: ";for(int i=0;i<26;++i)cout<<od[i]<<" ";cout<<endl;
        bool jz=0;
        int stp=-1;
        for(int i=0;i<26;++i){
            if(od[i]==id[i]+1){
            ...
            else if(od[i]>id[i]+1)jz=1;
        }
        if(jz){cout<<"The door cannot be opened.\n";continue;}
        if(stp!=-1)++id[stp],dfs(stp);
        else{
            for(int i=0;i<26;++i)if(od[i]){
                ++id[i],dfs(i);
                break;
            }
        }
        int unu=0;
        for(int i=0;i<n;++i)if(!us[i])++unu;
        if(unu){cout<<"The door cannot be opened.\n";continue;}
