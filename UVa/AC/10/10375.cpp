...
void init(){
    ...
    for(int i=2;i*i<=10003;++i){
        if(np[i])continue;
        for(int j=i*i;j<=10003;j+=i)np[j]=1;
    }
}
void meow(int v,int t){
    for(int i=2;i<=v;++i){
        int p=i;
        for(int j=2;;++j){
            if(!np[p]){
...
ld pw(ld b,int n,ld a=1,bool fl=0){
    if(n<0)fl=1,n=-n;
    while(n){
        if(n&1)a*=b;
        b*=b, n>>=1;
    } if(fl)return 1/a;return a;
}
int main(){
    init();
    int p,q,r,s;while(cin>>p>>q>>r>>s){
        meow(p,1);
        meow(q,-1);
        meow(p-q,-1);
        meow(r,-1);
        ...
        meow(r-s,1);
        ld ans=1.0;
        // for(int i=0;i<=10;++i)cout<<ps[i]<<" ";cout<<endl;
        for(int i=2;i<=10002;++i){
            if(ps[i])ans*=pw((ld)i,ps[i]);
            ps[i]=0;
        }
