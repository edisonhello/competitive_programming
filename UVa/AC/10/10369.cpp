...
int main(){
    int ts;cin>>ts;while(ts--){
        ...
        for(int i=0;i<p;++i){
            for(int j=i+1;j<p;++j){
                eg.push_back({i,j,(x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j])});
            }
        }
        sort(eg.begin(),eg.end(),[](const edge &a,const edge &b)->bool{return a.w<b.w;});
        int counter=p-s; djs.init(p);
        int mx=0;
        for(auto i:eg){
            if(djs.C(i.u,i.v))continue;
            djs.U(i.u,i.v); mx=max(mx,i.w); --counter;
            if(counter==0)break;
        }
        cout<<fixed<<setprecision(2)<<sqrt((ld)mx)<<endl;
    }
}
