...
int main(){
    int co,ca,sh;while(cin>>co>>ca>>sh){
        int td=co+ca;
        ld chfco=(ld)co/(ld)td;
        ld chfca=(ld)ca/(ld)td;
        ld cotoca=chfco*((ld)ca/(ld)(td-1-sh));
        ld catoca=chfca*((ld)(ca-1)/(ld)(td-1-sh));
        cout<<fixed<<setprecision(5)<<cotoca+catoca<<endl;
    }
}
