...
ld x[111],y[111],d[111]; bool v[111];
ld dist(int i,int j){return sqrt((x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j]));}
int main(){
    int ts;cin>>ts;while(ts--){
        int n;cin>>n; MS(v,0);
        for(int i=0;i<n;++i)cin>>x[i]>>y[i],d[i]=1e9;
        d[0]=0.0; ld tot=0.0; int nwpt=0;
        for(int _=0;_<n;++_){
            ld mnl=1e9;int pt;
            for(int i=0;i<n;++i){
                if((!v[i]) && mnl>d[i]){
            ...
            tot+=mnl; nwpt=pt; v[pt]=1;// cout<<pt<<" ";el;
            for(int i=0;i<n;++i){
                if(!v[i])d[i]=min(d[i],dist(pt,i));
            }
            // for(int i=0;i<n;++i)cout<<d[i]<<" ";el;
        }
