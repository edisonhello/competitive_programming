...
struct blk{int x,y,v;};
bool operator<(const blk &a,const blk &b){return a.v<b.v;}
PQ<blk,vector<blk>,less<blk>> pq;
...
int main(){
    ...
    int ts;cin>>ts;while(ts--){
        ...
        int ans=0;
        while(pq.size()){
            blk now=pq.top(); pq.pop();
            dp[now.x][now.y]=max({
                mp[now.x][now.y-1]>mp[now.x][now.y]?dp[now.x][now.y-1]:0,
                mp[now.x][now.y+1]>mp[now.x][now.y]?dp[now.x][now.y+1]:0,
                mp[now.x-1][now.y]>mp[now.x][now.y]?dp[now.x-1][now.y]:0,
                mp[now.x+1][now.y]>mp[now.x][now.y]?dp[now.x+1][now.y]:0})+1;
