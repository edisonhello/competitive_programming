...
int __buffer__[12345],dp[55][55],w[55][55];

int main(){
    // FIN,FOUT;
    int ts,ks=0;cin>>ts;cin.ignore();while(ts--){
        string s,t;getline(cin,s),getline(cin,t);s="!"+s,t="@"+t;
        memset(dp,0,sizeof(dp)); memset(w,0,sizeof(w));
        for(int i=1;i<s.length();++i)dp[i][0]=i,w[i][0]=1;
        for(int j=1;j<t.length();++j)dp[0][j]=j,w[0][j]=1;
        w[0][0]=1; for(int i=1;i<s.length();++i){
            for(int j=1;j<t.length();++j){
                if(s[i]==t[j])dp[i][j]=dp[i-1][j-1]+1,w[i][j]=w[i-1][j-1];
                else{
                    dp[i][j]=min(dp[i-1][j],dp[i][j-1]);
                    if(dp[i][j]==dp[i-1][j])w[i][j]+=w[i-1][j];
                    if(dp[i][j]==dp[i][j-1])w[i][j]+=w[i][j-1];
                    ++dp[i][j];
                }
            }
        }
        ...
        //     } cout<<endl;
        // }
        cout<<"Case #"<<(++ks)<<": "<<dp[s.length()-1][t.length()-1]<<" "<<w[s.length()-1][t.length()-1]<<endl;
    }
}
