...
int main(){
    string mp;while(cin>>mp){
        MS0(dp); int n=mp.length();
        ll ans=0;
        if(mp[0]=='?')for(int i=1;i<=n;++i)dp[1][i]=1;
        else if(meow(mp[0])>n)goto is0;
        else dp[1][meow(mp[0])]=1; 
        for(int i=2;i<=n;++i){
            if(mp[i-1]=='?'){
                for(int j=1;j<=n;++j){
                    for(int k=1;k<=n;++k){
                ...
                int j=meow(mp[i-1]); if(j>n)goto is0;
                for(int k=1;k<=n;++k){
                    if(abs(j-k)<2)continue;
                    dp[i][j]+=dp[i-1][k];
                }
