...
int main(){
    int n;rit(n);while(n--){
        int t;rit(t);s1.insert(t);s2.insert(t);
    }
    int m;rit(m);while(m--){
        int t;rit(t);
        auto it1=s1.upper_bound(t);
        if(it1==s1.end())putchar_unlocked('X'),putchar_unlocked(' ');
        else printf("%d ",*it1);
        auto it2=s2.upper_bound(t);
        if(it2==s2.end())puts("X");
        else printf("%d\n",*it2);
    }
}
