...
void dfs(int now,int pa){
    ...
    for(int i:G[now]){
        ...
        if(!num[i]){
            ...
            if(low[i]>=num[now])++dea;
        }
        else{
            low[now]=min(low[now],num[i]); // ?
        }
    }
    if(low[now]<=num[now] && ((ch>=1 && pa!=now)||(pa==now && ch>1)) && dea){
        cam.insert(im[now]);
    }
}

int main(){
    int n,ks=0;cin>>n;while(n){
        MS(num,0), MS(low,0), cnt=0;
        m.clear(), im.clear(); cam.clear();
