...
int main(){
    ...
    bool firstCase=1;
    string s;while(getline(cin,s) && s[0]!='0'){
        int n;{SS ss(s);ss>>n;}
        while(getline(cin,s) && s[0]!='0'){
            SS ss(s); for(int i=1;i<=n;++i)ss>>a[i];
            stack<int> stn; int ptr=1;
            for(int cm=1;cm<=n;++cm){
                while(stn.size() && ptr<=n && stn.top()==a[ptr]){stn.pop();++ptr;}
                if(a[ptr]==cm){++ptr;continue;}
                else stn.push(cm);
            }
            while(stn.size() && ptr<=n && stn.top()==a[ptr]){stn.pop();++ptr;}
            if(ptr>n)cout<<"Yes\n"; else cout<<"No\n";
