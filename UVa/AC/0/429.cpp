...
string dic[222];int n;
int G[222][222];

bool dif1(int i,int j){
    if(dic[i].length()!=dic[j].length())return 0;
    int dif=0;
    for(int k=0;k<dic[i].length();++k){
        if(dic[i][k]!=dic[j][k])++dif;
    }
