...
int a[100005];
vint G[100005],sG[100005];
int dp[100005],sdp[100005];

void dfs(int now){
    if(dp[now])return;
    int g=a[now],sg=a[now];
    for(int i:G[now]){
        dfs(i);
        g=max({g,a[now]+dp[i]});
        sg=max({sg,a[now]+sdp[i]});
    }
    for(int i:sG[now]){
        dfs(i);
        sg=max({sg,a[now]+dp[i]});
    }
    dp[now]=g, sdp[now]=sg;
}

int main(){
    int n,m,i,mx=0;rit(n,m);
    for(i=1;i<=n;++i)rit(a[i]);
    i=0;for(int a,b,t;i<m;++i){
        rit(a,b,t);
        if(t==0)G[a].pb(b);
        else sG[a].pb(b);
    }
