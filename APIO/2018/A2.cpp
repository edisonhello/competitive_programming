...
bool comp(const func &a,const func &b){
    return a.get(now_ref_point)==b.get(now_ref_point)?
                a.a==b.a?
                    a.b==b.b?
                        a.tt<b.tt:
                    a.b<b.b:
                a.a<b.a:
            a.get(now_ref_point)>b.get(now_ref_point);
}
...
//        if(l==1 && r==1)cout<<"this is a [1,1], size: "<<now->funcs.size()<<endl;
    ...
    if(ml<=l&&r<=mr){
        multiset<func,decltype(&comp)>::iterator i1,i2;
        now_ref_point=l;
        now->funcs.insert(ref_func);
        i1=now->funcs.upper_bound(ref_func);
        now->funcs.erase(now->funcs.find(ref_func));
        now_ref_point=r;
        now->funcs.insert(ref_func);
        i2=now->funcs.upper_bound(ref_func);
        if(i1!=i2){
            now->funcs.erase(now->funcs.find(ref_func));
