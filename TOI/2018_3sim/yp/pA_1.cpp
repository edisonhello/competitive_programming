...
int main ()
{
    ...
    if (k > 100)
    {
        ...
        for (int i=0;k>=i;i++)
        {
            ...
            {
                int j = order[jj];
                meruru2[i][ _[j] ] = max(meruru2[i][ _[j] ],meruru2[i][j] + lawfung[j][ _[j] ] );
                for (int gee:GG[j])
                {
                    if (gee == _[j]) continue;
                    meruru2[i+1][ gee ] = max(meruru2[i+1][gee],meruru2[i][j] + lawfung[j][gee]);
                }
            }
        }
        printf("%lld\n",meruru2[k][t]);
