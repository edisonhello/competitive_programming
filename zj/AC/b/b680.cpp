...
int main(){
    int n;cin>>n;
    for(int i=0;i<n;++i)cin>>a[i].num>>a[i].t;
    sort(a,a+n,[](const meow &a,const meow &b){return a.t<b.t;});
    // for(int i=0;i<n;++i)cout<<a[i].num<<" ";
    int ptr=0,li=0,pos=3;
    for(int d=1;ptr<n;){
        for(int _=0;_<n/8;++_)r[li++][pos]=a[ptr++].num;
        pos+=d++;
        for(int _=0;_<n/8;++_)r[--li][pos]=a[ptr++].num;
        pos-=d++;
    }
    for(int i=0;i<n/8;++i){
        cout<<i+1;for(int j=0;j<8;++j)cout<<" "<<r[i][j];
        cout<<endl;
    }
}
