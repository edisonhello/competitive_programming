...
pii p(string &s){
    int pos=s.find(':');
    SS ss1(s.substr(0,pos)),ss2(s.substr(pos+1,s.length()-pos-1));
    int i1,i2;ss1>>i1;ss2>>i2;
    return pii(i1,i2);
}
map<int,ll> d;
void add(string &s){
...
int main(){
    string s;getline(cin,s);SS ss(s);while(ss>>s)add(s);
    ll ans=0;
    string t;getline(cin,t);SS st(t);while(st>>t){
        pii v=p(t);
        PDE1(v);
        ans+=d[v.X]*v.Y;
    }
