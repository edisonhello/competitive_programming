...
void dfs(int now,int lft,vector<int> &v){
    if(now==n){
        if(lft!=0)return;
        ans.push_back(v);
        return;
    }
    for(int i=0;i*coin[now]<=lft;++i){
        v.push_back(i);
        dfs(now+1,lft-i*coin[now],v);
        v.pop_back();
    }
}
...
int main(){
    ...
    int co;cin>>co;
    sort(coin,coin+n); reverse(coin,coin+n);
    vint tmp;
    dfs(0,co,tmp);
    for(vint &v:ans)reverse(v.begin(),v.end());
    sort(ans.begin(),ans.end(),[](const vint &a,vint &b){
