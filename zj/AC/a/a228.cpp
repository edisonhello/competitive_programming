...
int main(){
    int ts,ks=0;cin>>ts;while(ts--){
        ...
        dp[n-1][m][0]=1;
        for(int i=n-1;i>=0;--i){
            if(i!=n-1)for(int j=0;j<(1<<m);++j)dp[i][m][j]=dp[i+1][0][j<<1];
            for(int j=m;j>0;--j){
                for(int type=0;type<(1<<m+1);++type){
                    int d=(1<<j)&type,r=(1<<j-1)&type;
                    PDE3(i,j,type);
                    PDE3(d,r,dp[i][j][type]);
                    if(!d && !r){
                        if(!ct[i][j-1])dp[i][j-1][type]+=dp[i][j][type];
                        else dp[i][j-1][type|(1<<j)|(1<<j-1)]+=dp[i][j][type];
                    }
                    if(d && !r){
                        if(ct[i][j-1]){
                            dp[i][j-1][type]+=dp[i][j][type];
                            dp[i][j-1][(type-d)|(1<<j-1)]+=dp[i][j][type];
                        }
                    }
                    if(!d && r){
                        if(ct[i][j-1]){
                            dp[i][j-1][type]+=dp[i][j][type];
                            dp[i][j-1][(type-r)|(1<<j)]+=dp[i][j][type];
                        }
                    }
                    if(d && r){
                        if(ct[i][j-1]){
