...
int main(){
    int n;cin>>n;
    string s,m;cin>>s>>m;
    sort(s.begin(),s.end());for(char c:s)++cs[c-'0'+1];
    sort(m.begin(),m.end());for(char c:m)++cm[c-'0'+1];
    int mn=0;for(int i=1;i<=10;++i){
        if(cs[i]>=cm[i])cs[i+1]+=cs[i]-cm[i];
        else mn+=cm[i]-cs[i];
    } MS0(cs),MS0(cm);
    sort(s.begin(),s.end());for(char c:s)++cs[c-'0'+1];
    sort(m.begin(),m.end());for(char c:m)++cm[c-'0'+1];
    int mx=0;for(int i=0,j=0;i<n;++i){
        while(m[j]<=s[i] && j<n)++j;
        if(!(j<n))break;
        ++mx;++j;
