...
ll query(ll n,ll l,ll r){
    // PDE3(n,l,r),FLH;
    ll hf=getLen(n>>1);
    if(l>hf*2+1)return 0;
    if(r>hf*2+1)return 0;
    if(n==0)return 0;
    if(n==1)return 1;
    PDE1(hf);
    if(r<=hf)return query(n>>1,l,r);
    if(l>=hf+2)return query(n>>1,l-hf-1,r-hf-1);
    if(r==hf+1)return query(n>>1,l,r-1)+(n&1);
    if(l==hf+1)return query(n>>1,l-hf,r-hf-1)+(n&1);
    return query(n>>1,l,hf)+(n&1)+query(n>>1,1,r-hf-1);
}
