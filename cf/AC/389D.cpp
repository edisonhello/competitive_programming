...
int main(){
    ...
    for(int i=0;i<ml;++i){
        if((1<<i)&k){
            int j=0;
            for(;j<i;++j){
                if(j==0)G[0].pb(p++),G[0].pb(p++);
                else{
                    G[p-2].pb(p), G[p-2].pb(p+1);
                    G[p-1].pb(p), G[p-1].pb(p+1);
                    p+=2;
                }
            }
            if(!makeLine){
                for(;j<ml-1;++j){
                    if(j==i && i)G[p-2].pb(p), G[p-1].pb(p);
                    else if(j==i && !i)G[0].pb(p);
                    else G[p-1].pb(p);
                    line[j]=p++;
                } makeLine=1;
                if(i==ml-1)G[p-2].pb(1), G[p-1].pb(1);
                else G[p-1].pb(1);
                line[ml-1]=1;
            }
            else G[p-2].pb(line[i]), G[p-1].pb(line[i]);
        }
    }
    cout<<p<<endl;
    for(int i=0;i<p;++i){
        for(int ii:G[i])c[i][ii]=1;
    }
    for(int i=0;i<p;++i){
        for(int j=0;j<p;++j){
            if(i==j)cout<<"N";
            else if(c[i][j] || c[j][i])cout<<"Y";
            else cout<<"N";
        } el;
    }
}
