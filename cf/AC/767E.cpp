...
bool ga[100005];
struct dy{int i,w;};
bool operator<(const dy &a,const dy &b){return a.w<b.w;}
bool operator>(const dy &a,const dy &b){return a.w>b.w;}
PQ<dy,vector<dy>,greater<dy>> pq;

int main(){
    // FIN;
    int n,c; rit(n,c);
    for(int i=0;i<n;++i)rit(cst[i]);
    ll ang=0;
    for(int i=0,w;i<n;++i){
        rit(w);
        c-=cst[i]%100;
        if(cst[i]%100)pq.push({i,(100-cst[i]%100)*w});
        if(c<0)c+=100,ang+=pq.top().w,ga[pq.top().i]=1,pq.pop();
    }
    pln(ang),el;
    for(int i=0;i<n;++i){
        if(ga[i])cout<<cst[i]/100+1<<" 0\n";
        else cout<<cst[i]/100<<" "<<cst[i]%100<<endl;
    }
}
