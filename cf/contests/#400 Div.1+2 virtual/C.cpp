...
int main(){
    cin>>n>>k;
    if(k==1)psbv.pb(1ll);
    else if(k==-1)psbv.pb(1ll),psbv.pb(-1ll);
    else for(ll kk=1;abs(kk)<=1000000009ll*100005ll;kk*=k)psbv.pb(kk);
    ll ans=0;
    mp[0]++;
    for(int i=0,t;i<n;++i){
        cin>>t; tot+=t;
        for(ll &j:psbv){
            auto it=mp.find(tot-j);
            if(it!=mp.end())ans+=it->Y;
        }
        mp[tot]++;
    } cout<<ans<<endl;
}
// 25101982 21:23 MLE18
// 25102022 22:47 MLE18
// 25102061 24:21 AC
