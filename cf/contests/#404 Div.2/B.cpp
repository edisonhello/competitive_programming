...
int main(){
    ...
    sort(a,a+n,[](const pii &a,const pii &b){return a.Y<b.Y;});
    sort(b,b+m,[](const pii &a,const pii &b){return a.X>b.X;});
    int ans=b[0].X-a[0].Y;
    sort(a,a+n,[](const pii &a,const pii &b){return a.X>b.X;});
    sort(b,b+m,[](const pii &a,const pii &b){return a.Y<b.Y;});
    ans=max(ans,a[0].X-b[0].Y);
    cout<<max(ans,0)<<endl;
}
// 25505006 08:00 AC
//          117:27 successful hack
