...
int LCA(int pa,int pb){
    if(dep[pb]>dep[pa])swap(pa,pb);
    for(int i=0;i<20;++i){
        if((dep[pa]-dep[pb])&(1<<i))pa=p[i][pa];
        if(dep[pa]==dep[pb])break;
    }
    if(pa==pb)return pa;
    for(int i=20;i>=0;--i){
...
int l(int a,int b,int c){
    ...
    if(bc==a)return 0;
    if(ab==a && ac==a && dep[bc]>dep[a])return dep[bc]-dep[a];
    return -(2*dep[LCA(bc,a)]-dep[bc]-dep[a]);
}
...
int main(){
    ...
    for(int i=2;i<=n;++i){
        int t;cin>>t;
        G[t].push_back(i); G[i].push_back(t);
    }
    dfs(1,0,0);
    while(q--){
        int a,b,c; cin>>a>>b>>c;
        // int ab=LCA(a,b),ac=LCA(a,c),bc=LCA(b,c);
        PDE4(a,b,c,l(c,a,b));
        cout<<max({l(a,b,c),l(a,c,b),l(b,a,c),l(b,c,a),l(c,a,b),l(c,b,a)})<<endl;
    }
}
