...
const ld eps=1e-8;
const ll mod=12345678923ll;

ll nor(ll x){return (x%mod+mod)%mod;}

int z[10018];
ll hsh[5008],pr[5008],p=47017;
void solve(int n,string &s){
    int l=0,r=0;
    for(int i=0;i<n;++i){
        z[i]=(i<=r?min(min(z[i-l-1],z[l-]),r-i):0);
        PDE4(i,l,r,z[i]);
...
int kp[5008][5009],cnt[88];
int main(){
    string s; cin>>s;
    string ss; for(char c:s)ss+=c,ss+=','; ss.pop_back();
    PDE1(ss);
    solve(ss.length(),ss);
    for(int i=0;i<ss.length();++i)cout<<z[i]<<" ";
    for(int i=0;i<ss.length();++i){
        if(i%2){
            for(int j=0;j<z[i];j+=2){
                kp[i/2][j/2]=1;
                cnt[1]++;
            }
        }
