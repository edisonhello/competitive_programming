...
int main(){
    int n,m,k; cin>>n>>m>>k;
    for(int i=1;i<=n;++i)cin>>thisis[i],appat[thisis[i]]=i;
    // for(int j=1;j<=n;++j)cout<<thisis[j]<<" ";cout<<endl;
    ll ans=0; while(m--){
        int i;cin>>i;
        ans+=(appat[i]-1)/k+1;
        if(appat[i]==1)continue;
        swap(thisis[appat[i]],thisis[appat[i]-1]);
        appat[i]--; appat[thisis[appat[i]+1]]++;
        // for(int j=1;j<=n;++j)cout<<thisis[j]<<" ";cout<<endl;
    } cout<<ans<<endl;
}
// 25211081 20:35 RE 33 (line: 132)
// 25211195 23:09 AC
