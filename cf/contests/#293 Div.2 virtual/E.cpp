...
int main(){ CIO;
    ...
    for(int i=0;i<n-k;++i){
        if(!unk[i] && !unk[i+k]){
            ll tdf=a[i+k]-a[i];
            if(fdf){
                if(tdf!=df)return cout<<"Incorrect sequence\n",0;
            }
            else fdf=1, df=tdf;
        }
    }
    for(int i=0;i<k;++i){
        ll base=0;int dist=-1;
        for(int j=i,d=0;j<n;j+=k,++d){
            if(!unk[j]){
                dist=d;
                base=a[j];
                break;
        ...
        // PDE4(dist,base,df,fdf);
        if(dist==-1){
            if(df==0)base=0,dist=0;
            else{
                int nums=(n-i-1)/k;
                base=0,dist=nums/2;
            }
        }
        // PDE4(i,df,base,dist);FLH;
        for(int j=i,d=0;j<n;j+=k,++d){
            if(unk[j]){
                unk[j]=0;
                a[j]=base+df*(d-dist);
...
// 25213626 75:42 WA 3
// 25215168 111:25 WA 3
// 25215426 118:01 WA 4
