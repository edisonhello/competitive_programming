...
int n;
struct pp{ld x,v;} a[60009];
bool operator<(const pp &a,const pp &b){
    return a.x<b.x;
}

bool canrch(ld t){
    ld l=-99999999999999999999.0,r=99999999999999999999.0;
    for(int i=0;i<n;++i){
        ld lll=a[i].x-a[i].v*t;
        ld rrr=a[i].x+a[i].v*t;
        l=max(lll,l);
        r=min(rrr,r);
        if(l>r)return 0;
    } return 1;
...
int main(){
    ...
    sort(a,a+n);
    // for(int i=0;i<n;++i)cout<<"("<<a[i].x<<", "<<a[i].v<<") ";cout<<endl;
    ld L=0.0,R=9999999999.0;
    while(R-L>1e-10){
        ld M=(L+R)/2;
        if(canrch(M))R=M;
        else L=M;
