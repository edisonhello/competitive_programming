...
int cnt[555],fstapp[555];

int main(){
    // freopen("in","r",stdin);
    // freopen("out","w",stdout);
    string s;cin>>s;
    for(int i=0;i<266;++i)fstapp[i]=-1;
    int _ptr=0;for(char c:s){
        cnt[c-'a']++;
        if(cnt[c-'a']==1)fstapp[c-'a']=_ptr;
        ++_ptr;
    }
    bool zero=0;
    for(int i=0;i<26;++i){
        if(cnt[i] && !zero)continue;
        if(cnt[i] && zero){
            cout<<"NO"<<endl;
            return 0;
        }
    ...
    int ptr=0;
    for(int i=0;i<26;++i){
        if(fstapp[i]!=-1 && fstapp[i]<ptr){
            cout<<"NO"<<endl;
            return 0;
        }
        ptr=max(ptr,fstapp[i]);
    }
    cout<<"YES\n";
}
// 24730611 16:47 WA 5
