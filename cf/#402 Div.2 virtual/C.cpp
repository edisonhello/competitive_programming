...
struct pd{int b,a;} a[200005];
bool operator<(const pd &a,const pd &b){return a.a-a.b>b.a-b.b;}

int main(){
    int n,k;cin>>n>>k;
    for(int i=0;i<n;++i)cin>>a[i].b;
    for(int i=0;i<n;++i)cin>>a[i].a;
    sort(a,a+n); int C=0,i=0;
    // for(int i=0;i<n;++i)cout<<a[i].a<<" "<<a[i].b<<" , ";cout<<endl;
    for(;i<k;++i){
        C+=a[i].b;
    }
    for(;i<n && a[i].a>a[i].b;++i){
        C+=a[i].b;
    }
    for(;i<n;++i){
        C+=a[i].a;
