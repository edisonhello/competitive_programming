...
bool chk(int q){
    // PDE1(q);
    for(int i=0;i<t.length();++i)ab[i]=1;
    for(int i=0;i<q;++i)ab[a[i]]=0;
    int i=0,j=0;
    // for(int i=0;i<t.length();++i)cout<<ab[i]<<" ";cout<<endl;
    for(;j<p.length() && i<t.length();++j,++i){
        while(i<t.length() && (t[i]!=p[j] || !ab[i])){
            ++i;
        }
        // PDE2(i,j);
        if(i>=t.length())return 0;
    }
    if(i>=t.length() && j<p.length())return 0;
    return 1;
}

int main(){
    cin>>t>>p;
    for(int i=0;i<t.length();++i)cin>>a[i],--a[i];
    int L=0,R=t.length();
    while(R>L){
        int M=(L+R)>>1;
        if(chk(M))L=M+1;
