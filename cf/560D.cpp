...
inline ll nmlz(ll n){return (n%mod+mod)%mod;}

const ll p=40801;
ll _buffer_[100],spfx[200005],_buffer__[100],tpfx[200005],__buffer_[100],ppw[200005];
string s,t;
int n;
...
int main(){
    cin>>s>>t;
    n=s.length();
    ppw[0]=1; spfx[0]=s[0], tpfx[0]=t[0];
    for(int i=1;i<n;++i){
        ppw[i]=ppw[i-1]*p%mod;
        spfx[i]=(spfx[i-1]+s[i]*ppw[i])%mod;
        tpfx[i]=(tpfx[i-1]+t[i]*ppw[i])%mod;
    }
