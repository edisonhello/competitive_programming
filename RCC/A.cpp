...
int a[33][33];
struct meow{int x,y;};
bool operator<(const meow &a,const meow &b){return a.x+a.y<b.x+b.y;}
bool operator>(const meow &a,const meow &b){return a.x+a.y>b.x+b.y;}
int main(){
    int t;cin>>t;while(t--){
        int n,m;cin>>n>>m;
        MS0(a);
        int ptr=n*m;
        priority_queue<meow,vector<meow>,greater<meow>> pq;
        for(int i=0;i<n;++i)for(int j=0;j<m;++j)pq.push({i,j});
        while(pq.size()){
            a[pq.top().x][pq.top().y]=ptr; ptr--; pq.pop();
        }
        for(int i=0;i<n;++i){
            for(int j=0;j<m;++j){
                cout<<a[i][j]<<" ";
