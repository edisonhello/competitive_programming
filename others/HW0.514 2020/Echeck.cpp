...
vector<pair<int, int>> solve(int n, int m, string s, vector<int> q) {
	s = " " + s;
	...
	for (int x : q) {
		if (ans[x].first == -1) rt.emplace_back(-1, -1);
		else rt.push_back(ans[x]);
	}
...
int main() {
	...
	while (true) {
		int n = rand() % 3 + 1, m = rand() % 5 + 1;
		cout << n << ' ' << m << '\n';
		string s;
		for (int i = 0; i < n; ++i) s += "TW"[rand() & 1];
		cout << s << endl;
		vector<int> q;
		for (int i = 1; i <= m; ++i) q.push_back(rand() % (2 * n) + 1);
		for (int i : q) cout << i << ' '; cout << endl;

		vector<pair<int, int>> rt = solve(n, m, s, q);
		...
		for (int i = 0; i < m; ++i) {
			...
			if (!ok[x]) {
				...
				assert(l != -1);
				int c = 0;
				for (int i = l; i <= r; ++i) c += s[i - 1] == 'T' ? 2 : 1;
				assert(c == x);
			}
		}
