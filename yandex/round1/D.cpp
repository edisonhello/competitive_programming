...
ll gethh(int l,int r){
    ...
    return ((hsh[r]-(l?hsh[l-1]:0))%mod+mod)%mod*pp[s.size()-r]%mod;
}
...
bool test(int l,int r){
    ...
    while(ptr<s.size()){
        ...
        /* while(R<s.size()){
            ...
            L=max(L,ptr);
            PDE(L,R,r-(R-L+1)+1,r);
            if(gethh(L,R)==gethh(r-(R-L+1)+1,r)){
                PDE("same");
                ptr=R+1;
                ok=1;
                break;
...
int main(){
    ...
    hsh[0]=s[0];
    pp[0]=1;
    for(int i=1;i<1234;++i)pp[i]=pp[i-1]*p%mod;
    for(int i=1;i<n;++i){
        hsh[i]=(hsh[i-1]+s[i]*pp[i]%mod)%mod;
    }
    for(int i=0;i<n;++i)PDE(i,hsh[i]);
    PDE(gethh(0,0),gethh(1,1));
    set<ll> tested;
    vector<string> ans;
    for(int i=0;i<s.size();++i){
        for(int j=i;j<s.size();++j){
            ll hh=gethh(i,j);
            if(tested.find(hh)!=tested.end())continue;
            tested.insert(hh);
            if(test(i,j))ans.push_back(s.substr(i,j-i+1));
        }
    }
