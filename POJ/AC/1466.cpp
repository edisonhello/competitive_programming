...
int main(){
    int n; while(cin>>n){
        ...
        for(int i=0;i<n;++i){
            getline(cin,s); int ptr=0; while(ptr<s.length() && s[ptr]!=' ')++ptr; ++ptr; while(ptr<s.length() && s[ptr]!=' ')++ptr; 
            if(ptr<s.length()){
                s=s.substr(ptr,s.length()-ptr);
            PDE1(s);
                SS ss(s); int j; while(ss>>j)G[i].push_back(j);
            }
        }
